...
- 不刪表:僅 `CREATE SCHEMA IF NOT EXISTS` + `CREATE OR REPLACE VIEW`;欄位集合有變動
  導致 Postgres 拒絕 REPLACE(既有 view 欄名 / 順序不可變更)時,退回「DROP VIEW IF
  EXISTS + CREATE VIEW」重建流程 — 僅限本情境,且僅 DROP VIEW,不動任何表
  (`04-sql-safety.md` 禁字串拼接;識別字全走白名單 `quote_ident`)。
- 重生觸發:mapping「內容簽名」(排序後正規化字串的 sha256)與上次相比有變才重生,否則
  略過。刻意不用本地 `updated_at`(整表重灌每輪都會刷新該欄位,無法反映內容是否真的
  變動);簽名存 Redis(對齊既有 `app.core.redis` 快取慣例)。
"""
...
async def _create_or_replace_view(
    ...
    """先嘗試 `CREATE OR REPLACE VIEW`;欄位集合變動導致 Postgres 拒絕(既有 view 欄名 /
    順序不可變更)時,退回「DROP VIEW IF EXISTS + CREATE VIEW」重建流程 — 僅限本情境,
    且僅 DROP VIEW,不動任何表(對齊 `04-sql-safety.md` 底線,不刪表)。
    """
    try:
        await conn.execute(text(f"CREATE OR REPLACE VIEW {qualified_view} AS {select_sql}"))
    except Exception:
        logger.info("CREATE OR REPLACE VIEW 欄位集合不相容,改走重建流程:%s", qualified_view)
...
async def generate_views_for_schema(
    ...
    回傳實際建立 / 更新的 view 全名(`<schema>_en.<view_name>`)清單。單表失敗不中斷同
    schema 其餘表(對齊 mirror_sync 單表失敗不中斷整輪的慣例)。
    """
    created: list[str] = []
...
async def generate_views(
    ...
    """迴圈目標 RDS 實際存在的帳套 schema,依 confirmed 映射產生語意化 view。回傳建立清單。

    連線以 AUTOCOMMIT 執行(逐表 DDL 各自即時生效):單表失敗不會讓交易中止而波及
    同一 schema / 其餘 schema 尚未處理的表。
    """
    created: list[str] = []
    async with engine.connect() as conn:
        ...
        for schema in schemas:
            created.extend(await generate_views_for_schema(conn, schema, confirmed))
    return created
...
async def regenerate_views_if_changed(
    ...
    try:
        created = await generate_views(engine, confirmed)
    finally:
        await engine.dispose()
    await cache.cache_set(_SIGNATURE_CACHE_KEY, signature, ttl_seconds=_SIGNATURE_TTL_SECONDS)
    logger.info("view 重生完成:%d 個 view", len(created))
