...
- 授權來源 = **角色綁定的角色權限設定檔 ∪ 未過期的臨時權限組**(加法模型,只授予、無 deny)。
...
- 「開門」與「授權」的配對**以授權來源為單位**判定(角色權限設定檔一組、每個臨時權限組各一組),
  避免 A 來源開門、B 來源給欄位被拼裝成雙方都沒真正授予的權限;各來源結果再聯集。
- 效期比較走 naive UTC+8(`04-databases/06-timezone.md`),過期臨時權限由 repo 層直接濾掉。

讀取走 task-003 快取(`client_setting:effective:<client_uid>`,TTL 兜底,Redis 故障降級直讀)。
快取內容含角色 / 臨時權限組名稱與效期:名稱改名、以及「快取後才到期」的臨時權限,最遲於 TTL
...
# 單筆授權項:作業 pid × 表 × 欄位 × 動作(角色權限設定檔 / 臨時權限組共用同一形狀)
...
async def _load_role(
    ...
    """取 Client 的角色與其角色權限設定檔 pid;無指派 / 角色已刪 → 該側無任何授權。"""
