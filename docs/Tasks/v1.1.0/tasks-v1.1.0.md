...
| 003 | DF-SSO 後端整合(雙軌之 SSO 側) | in_progress(worker: claude-A) | ✗ | 002 | `backend/app/api/v1/sso.py` / `api/v1/__init__.py` / `clients/df_sso.py` / `services/sso_service.py` / `core/config.py` / `tests/test_sso.py` |
