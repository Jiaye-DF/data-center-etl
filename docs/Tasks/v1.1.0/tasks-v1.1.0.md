...
| 006 | ETL 執行核心(純 Python,DB 設定驅動 + 逐表詳細 log) | pending | ✓ | 001 | `backend/app/etl/`(engine/reader/writer/comments/transforms)/ `tests/test_etl_engine.py` / `tests/test_etl_transforms.py` |
| 007 | taskiq + redis 排程服務(broker/worker/scheduler) | pending | ✓ | 006 | `backend/app/worker/`(broker/tasks/scheduler)/ `tests/test_worker.py` |
| 008 | v1.0.0 mapping 設定匯入自有 DB(seed) | pending | ✓ | 001 | `backend/scripts/seed_etl_config.py` / `tests/test_seed_etl_config.py` |
