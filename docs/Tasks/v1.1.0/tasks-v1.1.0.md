...
| 005 | 排程 / 執行紀錄與詳細 log / 手動觸發 API | done(worker: claude-A) | ✗ | 004,007 | `backend/app/api/v1/schedules.py` / `api/v1/runs.py` / `api/v1/__init__.py` / `schemas/schedule.py` / `schemas/run.py` / `services/schedule_service.py` / `repositories/schedule_repo.py` / `repositories/run_repo.py` / `tests/test_schedule_api.py` / `tests/test_runs_api.py` |
