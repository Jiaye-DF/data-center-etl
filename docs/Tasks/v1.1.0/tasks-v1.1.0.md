...
| 009 | 前端登入頁(雙軌)+ 後台佈局殼 + auth guard | in_progress(worker: claude-D) | ✓ | 002,003 | `frontend/src/app/login/page.tsx` / `middleware.ts` / `app/(main)/layout.tsx` / `app/(main)/page.tsx` / `lib/api/authApi.ts` / `lib/auth/useAuth.ts` / `app/error.tsx` / `app/global-error.tsx` |
