...
| 004 | ETL 設定管理 API(表清單/啟停/mapping/Comment) | done(worker: claude-A) | ✗ | 001,003 | `backend/app/api/v1/etl_tables.py` / `api/v1/__init__.py` / `schemas/etl_config.py` / `services/etl_config_service.py` / `repositories/etl_config_repo.py` / `tests/test_etl_config_api.py` |
| 005 | 排程 / 執行紀錄與詳細 log / 手動觸發 API | in_progress(worker: claude-A) | ✗ | 004,007 | `backend/app/api/v1/schedules.py` / `api/v1/runs.py` / `api/v1/__init__.py` / `schemas/schedule.py` / `schemas/run.py` / `services/schedule_service.py` / `repositories/schedule_repo.py` / `repositories/run_repo.py` / `tests/test_schedule_api.py` / `tests/test_runs_api.py` |
