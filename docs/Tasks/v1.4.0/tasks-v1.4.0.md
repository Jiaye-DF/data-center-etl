...
| 003 | RBAC 後端 — 資料層端點全面 require_admin | in_progress(worker: claude-B) | ✓ | — | `backend/app/api/v1/{datasets,schedules,runs,sync,dashboard,audit_logs}.py` / `backend/tests/{test_sync_api,test_runs_api,test_schedule_api_v131,test_snapshot_service,test_audit_log}.py` |
