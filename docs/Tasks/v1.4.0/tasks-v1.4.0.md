...
| # | 標題 | 狀態 | 並行 | 依賴 | 影響檔案 |
| --- | --- | --- | --- | --- | --- |
| 001 | 同輪表級平行同步(worker 併發 + SYNC_CONCURRENCY env) | pending | ✓ | — | `backend/app/worker/tasks.py` / `backend/app/core/config.py` / `backend/tests/test_mirror_sync_parallel.py` / 3×`docker-compose*.yml` / 3×`.env*.example` |
...
| 003 | RBAC 後端 — 資料層端點全面 require_admin | pending | ✓ | — | `backend/app/api/v1/{datasets,schedules,runs,sync,dashboard,audit_logs}.py` / `backend/tests/{test_sync_api,test_runs_api,test_schedule_api_v131,test_snapshot_service,test_audit_log}.py` |
| 004 | RBAC 前端 — member 無權限頁 + 路由守衛 + 導覽隱藏 | pending | ✓ | — | `frontend/src/app/(main)/layout.tsx` / `frontend/src/app/(main)/no-access/page.tsx`(新) / `frontend/src/components/layout/Sidebar.tsx` |
