...
- `services/snapshot_service.py`:
  - `refresh(dataset)`:呼叫 introspect 列 schema/table/column_count/row_count → upsert repo → 回統計;寫 `snapshot_at`。
  - `list_schemas(dataset)` / `list_tables(dataset, schema, page, page_size, hide_empty)`:**讀 repo**,Redis cache(key 含 dataset/schema/page/hide_empty);refresh 後失效對應 key。
- `api/v1/datasets.py`:
...
- `schemas/rawdata.py`:`TableSummary` 加 `last_synced_at` / `last_transformed_at`(nullable ISO);移除 `ColumnListResponse` 若無他用(或保留但不掛路由)。
...
- [ ] `curl -s -X POST -b <admin cookie> localhost:8000/api/v1/datasets/source/snapshot/refresh | jq -e '.success == true'`,且刷新後 `rds_table_meta` DS 筆數 == 來源實際表數
