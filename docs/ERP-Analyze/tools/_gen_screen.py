# -*- coding: utf-8 -*-
"""整合版產生器：DS 字典（GAT/GAQ/ZZ/GAZ/ZR/GAE）↔ M2201 表/欄位 ↔ TipTop 畫面(程式)。
由 _data/*.tsv 產出 docs/output/erp-metadata.md + erp-metadata-m2201.html（覆蓋原版）。
資料來源皆為先前以唯讀 SELECT 抽出的 TSV，本腳本不連 DB。"""
import os, html, collections, io, sys, bisect

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
# 目錄整併後版面:本檔在 tools/,資料在 ../data、輸出在 ../output(docs/ERP-Analyze/ 之下)
BASE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(BASE)
DATA = os.path.join(ROOT, "data")
OUT  = os.path.join(ROOT, "output")
os.makedirs(OUT, exist_ok=True)
TODAY = "2026-07-17"

def read_tsv(name):
    """單純以 tab 切割（資料含引號字元，避免 csv 引號規則吞行）。"""
    rows = []
    with open(os.path.join(DATA, name), encoding="utf-8") as f:
        header = f.readline().rstrip("\r\n").split("\t")
        for line in f:
            vals = line.rstrip("\r\n").split("\t")
            if len(vals) < len(header):
                vals += [""] * (len(header) - len(vals))
            rows.append(dict(zip(header, vals)))
    return rows

owners   = read_tsv("owners.tsv")              # OWNER TABS TABS_WITH_DATA TOTAL_ROWS LAST_STAT
tables   = read_tsv("m2201_tables.tsv")        # TABLE_NAME NUM_ROWS ZH_NAME MODULE HAS_PK
columns  = read_tsv("m2201_columns.tsv")       # TABLE_NAME COLUMN_NAME DATA_TYPE ... ZH_NAME PK_POS
modules  = read_tsv("m2201_modules.tsv")       # MODULE ...
programs = read_tsv("ds_programs.tsv")         # PROG MODULE PTYPE ZH_NAME
zr       = read_tsv("ds_zr.tsv")               # ZR01 ZR02 ZR03
gae      = read_tsv("ds_gae.tsv")              # GAE01 GAE02 GAE04
ds_tabs  = read_tsv("ds_tables.tsv")           # TABLE_NAME NUM_ROWS ZH_NAME MODULE（DS 全部 2,456 張）
perm_st  = read_tsv("ds_perm_structs.tsv")     # 權限相關表結構（TABLE_NAME COLUMN_ID COLUMN_NAME DATA_TYPE DATA_LENGTH ZH）
zy_rows  = read_tsv("ds_zy.tsv")               # ZY01 ZY02 ZY03 ZY04 ZY05 ZY07（權限設定 33k）
zw_rows  = read_tsv("ds_zw.tsv")               # ZW01 ZW02 ZW03（權限類別 57）
zxy_rows = read_tsv("ds_zxy.tsv")              # ZXY01 ZXY02 ZXY03（使用者↔營運中心）
gau      = read_tsv("ds_gau.tsv")              # GAU01=PK欄 GAU02=PK表 GAU03=FK欄 GAU04=FK表（1,957 條）

# 英文語意名草稿(erp_metadata.semantic_mappings 的種子;翻譯批次合併產生。無檔案時第 9 章顯示提示)
SEM_FILE = os.path.join(DATA, "semantic_draft.tsv")
semantic = read_tsv("semantic_draft.tsv") if os.path.exists(SEM_FILE) else []
sem_tab  = {r["TABLE_NAME"]: r for r in semantic if not r["COLUMN_NAME"]}   # 表層級(COLUMN_NAME 空)
sem_cols = [r for r in semantic if r["COLUMN_NAME"]]                        # 欄位層級

# ---- 權限模型統計（由抽出資料計算，屬事實）----
perm_cols_by_tab = collections.defaultdict(list)
for r in perm_st:
    perm_cols_by_tab[r["TABLE_NAME"]].append(r)
zy_tokens = collections.Counter()
zy_scope4 = collections.Counter()
zy_scope5 = collections.Counter()
for r in zy_rows:
    for tok in (r["ZY03"] or "").split(","):
        tok = tok.strip()
        if tok:
            zy_tokens[tok] += 1
    zy_scope4[(r["ZY04"] or "").strip()] += 1
    zy_scope5[(r["ZY05"] or "").strip()] += 1
zy_roles = len({r["ZY01"] for r in zy_rows})
zy_progs = len({r["ZY02"] for r in zy_rows})
zxy_users = len({r["ZXY01"] for r in zxy_rows})

# ---- 程式字典（DS.ZZ_FILE + DS.GAZ_FILE）----
prog_zh  = {p["PROG"]: (p["ZH_NAME"] or "").strip() for p in programs}
prog_mod = {p["PROG"]: (p["MODULE"] or "").strip() for p in programs}

ACT_ZH = {"I": "新增", "U": "更新", "D": "刪除", "S": "查詢"}
KIND_SCORE = {"i": 2, "t": 2, "m": 1}   # 程式代號第4碼：i=建檔 t=交易（TipTop 命名慣例，推導）

def prog_kind(p):
    return p[3].lower() if len(p) > 3 else ""

def rank_prog(prog, acts):
    w = len(acts & {"I", "U", "D"})
    name_score = 1 if "維護" in prog_zh.get(prog, "") else 0
    return (-w, -KIND_SCORE.get(prog_kind(prog), 0), -name_score, prog)

# ---- 表 ↔ 程式（DS.ZR_FILE）----
tbl_progs = collections.defaultdict(lambda: collections.defaultdict(set))
for r in zr:
    tbl_progs[r["ZR02"].strip().lower()][r["ZR01"].strip()].add(r["ZR03"].strip())

# ---- 欄位 ↔ 畫面欄位名稱（DS.GAE_FILE，繁中）----
label_by_form = collections.defaultdict(dict)
label_count   = collections.defaultdict(collections.Counter)
field_forms   = collections.defaultdict(set)
for r in gae:
    form, field, label = r["GAE01"].strip(), r["GAE02"].strip().lower(), (r["GAE04"] or "").strip()
    if not label:
        continue
    label_by_form[form][field] = label
    label_count[field][label] += 1
    field_forms[field].add(form)

forms_sorted = sorted(label_by_form.keys())
_prog_forms_cache = {}

def prog_forms(prog):
    """該程式的主畫面＋子畫面（gae01 以程式代號開頭），排序表 + bisect 取前綴區間。"""
    if prog not in _prog_forms_cache:
        lo = bisect.bisect_left(forms_sorted, prog)
        hi = bisect.bisect_right(forms_sorted, prog + "￿")
        _prog_forms_cache[prog] = forms_sorted[lo:hi]
    return _prog_forms_cache[prog]

def prog_label(prog, field):
    d = label_by_form.get(prog)
    if d and field in d:
        return d[field]
    for f in prog_forms(prog):
        if field in label_by_form[f]:
            return label_by_form[f][field]
    return ""

def all_labels(field):
    """該欄位在全系統畫面上的所有相異名稱：[(名稱, 出現畫面數)]，依畫面數排序。"""
    c = label_count.get(field)
    if not c:
        return []
    return c.most_common()

# ---- M2201 欄位 ----
cols_by_table = collections.defaultdict(list)
for c in columns:
    cols_by_table[c["TABLE_NAME"]].append(c)

def fmt_type(c):
    dt = c["DATA_TYPE"]
    if dt == "NUMBER":
        p, s = c.get("DATA_PRECISION", "").strip(), c.get("DATA_SCALE", "").strip()
        if p:
            return f"NUMBER({p}{','+s if s and s != '0' else ''})"
        return "NUMBER"
    if dt in ("VARCHAR2", "CHAR", "NVARCHAR2", "NCHAR"):
        return f"{dt}({c['DATA_LENGTH']})"
    return dt

MODULE_ZH = {
    "AGL":"總帳 General Ledger","GGL":"總帳(共用)","CGL":"總帳(集團)",
    "AAP":"應付帳款 Accounts Payable","APY":"請款/付款",
    "AXR":"應收帳款 Accounts Receivable","CXR":"應收(集團)","GXR":"應收(共用)","ARM":"應收沖帳",
    "AIM":"庫存管理 Inventory","ASM":"庫存異動","AMM":"物料管理","AMD":"物料需求","AMR":"物料","AMS":"物料",
    "APM":"採購管理 Purchasing","CPM":"採購(集團)","AQC":"品檢/品保",
    "AOO":"訂單/銷貨 Order/Sales","AXM":"銷售管理","COO":"訂單(集團)","CXM":"銷售(集團)",
    "AFA":"固定資產 Fixed Assets","ANM":"財務分錄/底稿","CNM":"財務分錄(集團)","GNM":"財務分錄(共用)",
    "AXC":"成本計算 Costing","ASF":"生產/製令 Shop Floor","CSF":"生產(集團)","APJ":"專案管理 Project",
    "ABX":"票據管理","ABG":"票據","ASR":"售後服務/維修","AWS":"工作站/整合站台","AZZ":"系統共用/雜項",
}
def mod_label(code):
    code = (code or "").strip()
    if code == "":
        return "(未分類)"
    return MODULE_ZH.get(code, code + "（推測）")

ROLE = {
 "DS":"**資料字典/系統設定**（GAT/GAQ/ZZ/GAZ/ZR/GAE 等字典表）","DS_REPORT":"報表暫存",
 "M2201":"**公司帳套（本帳號對應，主要分析對象）**",
 "SDF":"公司帳套（資料量最大）","GDF":"公司帳套","MDF":"公司帳套","DF":"公司帳套",
 "HW":"公司帳套","HWTAX":"公司帳套(稅)","FT":"公司帳套",
 "F2204":"公司帳套","S2202":"公司帳套","G2203":"公司帳套","TDF":"公司帳套",
 "NODATA":"範本/空帳套(推測)","SKL":"帳套(複本)","SKY":"帳套(複本)","SLE":"帳套(複本)",
 "SLY":"帳套(複本)","XCAR":"帳套(複本)","GLY":"帳套(複本)","SPA":"帳套(複本)",
 "DFTEST":"舊測試帳套(統計過期)","DFTEST2":"舊測試帳套","DFTEST6":"舊測試帳套","ERPMIS":"舊測試帳套",
 "PATCHTEMP":"程式更新暫存","SYS":"Oracle 系統","SYSTEM":"Oracle 系統","WMSYS":"Oracle Workspace Manager",
}

# ---- 單頭/單身族（命名慣例推導，沿用原 erp-metadata 邏輯）----
def prefix3(t):
    n = t[:-5] if t.endswith("_FILE") else t
    return n if len(n) == 3 else None

fam = collections.defaultdict(list)
for t in tables:
    p = prefix3(t["TABLE_NAME"])
    if p:
        fam[p[:2]].append(p)
families = {k: sorted(set(v)) for k, v in fam.items() if len(set(v)) > 1}
table_zh_map = {t["TABLE_NAME"]: t["ZH_NAME"] for t in tables}
rels = []
for fam2, prefs in sorted(families.items()):
    htab = prefs[0] + "_FILE"
    for p in prefs[1:]:
        rels.append((htab, p + "_FILE", fam2))
fam_detail = collections.defaultdict(list)
for h_, d_, f2 in rels:
    fam_detail[h_].append(d_)

# ---- 每表組裝（程式對應 + 欄位標籤）----
table_rows = []
n_col_total = n_col_dict = n_col_screen = 0
for t in tables:
    tname = t["TABLE_NAME"]
    progs = tbl_progs.get(tname.lower(), {})
    tbl_fields = [c["COLUMN_NAME"].lower() for c in cols_by_table.get(tname, [])]

    def screen_coverage(prog):
        forms = prog_forms(prog)
        if not forms:
            return 0
        return sum(1 for fld in tbl_fields if any(fld in label_by_form[f] for f in forms))

    coverage = {p: screen_coverage(p) for p in progs}
    ordered = sorted(progs.items(), key=lambda kv: (-coverage[kv[0]],) + rank_prog(kv[0], kv[1]))
    writers = [(p, a) for p, a in ordered if a & {"I", "U", "D"}]
    main_prog = writers[0][0] if writers else (ordered[0][0] if ordered else "")

    cols = []
    for c in cols_by_table.get(tname, []):
        field = c["COLUMN_NAME"].lower()
        main_lab = prog_label(main_prog, field) if main_prog else ""
        labs = all_labels(field)
        n_col_total += 1
        if c["ZH_NAME"]:
            n_col_dict += 1
        if main_lab or labs:
            n_col_screen += 1
        cols.append({
            "col": c["COLUMN_NAME"], "dict_zh": c["ZH_NAME"], "type": fmt_type(c),
            "null": "Y" if c["NULLABLE"] == "Y" else "N",
            "pk": c["PK_POS"], "main_lab": main_lab, "labs": labs,
            "nforms": len(field_forms.get(field, ())),
        })
    table_rows.append({
        "table": tname, "zh": t["ZH_NAME"], "module": t["MODULE"], "rows": int(t["NUM_ROWS"] or 0),
        "has_pk": t["HAS_PK"] == "1",
        "main_prog": main_prog, "main_prog_zh": prog_zh.get(main_prog, ""),
        "progs": ordered, "n_writers": len(writers), "cols": cols,
    })

by_rows   = sorted(table_rows, key=lambda x: (-x["rows"], x["table"]))
n_tbl = len(table_rows)
n_tbl_prog = sum(1 for t in table_rows if t["progs"])
n_tbl_writer = sum(1 for t in table_rows if t["n_writers"])

def fmt(n): return f"{n:,}"
def acts_str(a): return "/".join(x for x in "IUDS" if x in a)
def md_escape(s): return (s or "").replace("|", "\\|").replace("\n", " ")

# 模組分組（欄位清單章）
by_mod = collections.defaultdict(list)
for t in table_rows:
    by_mod[t["module"]].append(t)
order_mod = [m["MODULE"] for m in modules]
for m in by_mod:
    if m not in order_mod:
        order_mod.append(m)

# ---- 反查：主要程式 → 影響的資料表（來源 ZR_FILE，僅涵蓋 M2201 有資料的 333 張表）----
tbl_cols_l = {t["TABLE_NAME"]: [c["COLUMN_NAME"].lower() for c in cols_by_table.get(t["TABLE_NAME"], [])]
              for t in tables}
_prog_fields_cache = {}
def prog_fields(prog):
    """該程式（含子畫面）畫面上出現過的所有欄位代碼集合。"""
    if prog not in _prog_fields_cache:
        s = set()
        for f in prog_forms(prog):
            s.update(label_by_form[f].keys())
        _prog_fields_cache[prog] = s
    return _prog_fields_cache[prog]

prog_tbls = collections.defaultdict(list)
for t in table_rows:
    for p, a in t["progs"]:
        prog_tbls[p].append((t["table"], t["zh"], a, p == t["main_prog"]))
# 只列「對至少一張表有寫入(I/U/D)」的程式；純查詢程式數量太多且非維護主體
writer_progs = {p: lst for p, lst in prog_tbls.items() if any(x[2] & {"I", "U", "D"} for x in lst)}
def prog_sort_key(p):
    lst = writer_progs[p]
    n_main = sum(1 for x in lst if x[3])
    n_write = sum(1 for x in lst if x[2] & {"I", "U", "D"})
    return (-n_main, -n_write, p)
writer_order = sorted(writer_progs, key=prog_sort_key)

# ---- GAU_FILE（鼎新字典層 PK/FK）品質統計 ----
colset_l = {(c["TABLE_NAME"].lower(), c["COLUMN_NAME"].lower()) for c in columns}
tabset_l = {t["TABLE_NAME"].lower() for t in tables}
zh_any = {t["TABLE_NAME"].lower(): t["ZH_NAME"] for t in tables}
for t in ds_tabs:
    zh_any.setdefault(t["TABLE_NAME"].lower(), t["ZH_NAME"])
gau_pk_tabs = {r["GAU02"] for r in gau}
gau_fk_tabs = {r["GAU04"] for r in gau}
gau_both = [r for r in gau if r["GAU02"] in tabset_l and r["GAU04"] in tabset_l]
gau_ok = sum(1 for r in gau_both
             if (r["GAU02"], r["GAU01"]) in colset_l and (r["GAU04"], r["GAU03"]) in colset_l)
gau_cover = sum(1 for t in tabset_l if t in gau_pk_tabs or t in gau_fk_tabs)
gau_pairs = {(r["GAU02"], r["GAU04"]) for r in gau}
derived_pairs = {(h.lower(), d.lower()) for h, d, _ in rels}
gau_hit_derived = sum(1 for pr in derived_pairs if pr in gau_pairs)
gau_top = collections.Counter(r["GAU02"] for r in gau).most_common(15)

# ---- 機器可讀 TSV（維持先前輸出）----
def w_tsv(name, header, rows):
    with open(os.path.join(DATA, name), "w", encoding="utf-8", newline="") as f:
        f.write("\t".join(header) + "\n")
        for r in rows:
            f.write("\t".join(str(x) for x in r) + "\n")

w_tsv("m2201_table_programs.tsv", ["TABLE_NAME", "PROG", "PROG_ZH", "ACTIONS", "IS_MAIN"],
      [(t["table"], p, prog_zh.get(p, ""), acts_str(a), "Y" if p == t["main_prog"] else "")
       for t in by_rows for p, a in t["progs"]])
w_tsv("m2201_column_screen_labels.tsv",
      ["TABLE_NAME", "COLUMN_NAME", "DICT_ZH", "MAIN_PROG", "MAIN_PROG_LABEL", "ALL_SCREEN_LABELS", "N_FORMS"],
      [(t["table"], c["col"], c["dict_zh"], t["main_prog"], c["main_lab"],
        " | ".join(f"{lab}({n})" for lab, n in c["labs"]), c["nforms"] or "")
       for t in by_rows for c in t["cols"]])
# 一列一名稱的展開版（欄位 × 名稱 × 出現畫面數）
w_tsv("m2201_column_label_variants.tsv",
      ["TABLE_NAME", "COLUMN_NAME", "SCREEN_LABEL", "N_FORMS"],
      [(t["table"], c["col"], lab, n)
       for t in by_rows for c in t["cols"] for lab, n in c["labs"]])
# 程式 × 表 × 推導欄位（僅寫入 I/U/D 的 pair；欄位=表欄位 ∩ 程式畫面欄位，推導）
w_tsv("m2201_prog_table_columns.tsv",
      ["PROG", "TABLE_NAME", "ACTIONS", "COLUMN_NAME"],
      [(p, tbl, acts_str(acts), c)
       for p in writer_order
       for tbl, _zh, acts, _m in writer_progs[p] if acts & {"I", "U", "D"}
       for c in tbl_cols_l.get(tbl, []) if c in prog_fields(p)])

# ---- DS 字典表總表（本文件核心結論；筆數為驗證當時查得）----
DICT_TABLES = [
    ("DS.GAT_FILE","**資料表中文名檔**","GAT01=表名(小寫)、GAT02=語言別(0=繁中/2=简体)、GAT03=**表中文名**、GAT06=模組","2,430"),
    ("DS.GAQ_FILE","**欄位字典檔**（欄位中文名）","GAQ01=欄位名(小寫)、GAQ02=語言別、GAQ03=**欄位中文名**、GAQ04/05=說明/選項","54,196"),
    ("DS.ZZ_FILE","**程式資料檔**（所有作業/畫面的主檔）","ZZ01=程式代號、ZZ011=模組、ZZ03=程式類別","4,982"),
    ("DS.GAZ_FILE","**程式名稱多語言檔**（畫面/選單節點的中文名稱）","GAZ01=程式代號(含選單節點 m??)、GAZ02=語言別、GAZ03=**程式名稱**","5,596"),
    ("DS.ZR_FILE","**程式↔資料表關連檔**（哪支程式讀寫哪張表）","ZR01=程式、ZR02=表名、ZR03=異動別(I/U/D/S)","102,262"),
    ("DS.GAE_FILE","**畫面欄位檔**（畫面上每個欄位的顯示名稱）","GAE01=畫面檔代碼、GAE02=欄位代碼、GAE03=語言別、GAE04=**畫面欄位名稱**","158,187"),
    ("DS.ZM_FILE","**選單目錄檔**（ERP 左側選單樹）","ZM01=目錄代號(如 m2006)、ZM03=序號、ZM04=程式代號或子目錄；節點中文名查 GAZ_FILE","5,924"),
    ("DS.GAO_FILE","**模組執行路徑檔**（模組→部署目錄）","GAO01=模組代碼、GAO02=路徑（`$TOP`=標準版/`$CUST`=**客製模組**）、GAO05=財務模組否","107"),
    ("DS.GAU_FILE","主鍵/外鍵關聯紀錄檔（鼎新邏輯層 PK/FK，補充來源）","GAU01/02=PK 欄位/表、GAU03/04=FK 欄位/表","1,957"),
]

# ---- DS 全部資料表分類（推導：依表名前綴與中文名關鍵字）----
CORE_DICT = {"GAT_FILE","GAQ_FILE","ZZ_FILE","GAZ_FILE","ZR_FILE","GAE_FILE","ZM_FILE","GAO_FILE","GAU_FILE"}
def ds_category(name, zh):
    if name in CORE_DICT:
        return "★ 核心字典（本文件使用）"
    n = name.lower()
    z = zh or ""
    if "權限" in z or n.startswith("zx"):
        return "權限/使用者"
    if "流程" in z or "簽核" in z:
        return "流程/簽核"
    if "背景" in z or n.startswith("gd"):
        return "背景作業/排程"
    if "報表" in z or n.startswith("zab") or n.startswith("gar"):
        return "報表/列印設定"
    if n.startswith("ga") or n.startswith("gb") or "多語言" in z or "畫面" in z:
        return "系統字典/畫面/多語言"
    if n.startswith("z"):
        return "程式/選單/系統核心"
    if n.startswith("az"):
        return "系統共用設定"
    if n.startswith("p_") or "pat" == n[:3]:
        return "程式更新(Patch)"
    return "共用業務/其他"

JOIN_SQL = """-- 1) 表中文名（GAT_FILE）
SELECT gat03 表中文名 FROM ds.gat_file WHERE gat01 = 'oea_file' AND gat02 = '0';

-- 2) 欄位中文名（GAQ_FILE）
SELECT gaq01 欄位, gaq03 欄位中文名 FROM ds.gaq_file WHERE gaq01 LIKE 'oea%' AND gaq02 = '0';

-- 3) 某張表被哪些程式(畫面)使用（ZR_FILE + GAZ_FILE）
SELECT zr.zr01 程式代號, gaz.gaz03 程式名稱, zr.zr03 異動別
FROM   ds.zr_file zr
LEFT   JOIN ds.gaz_file gaz ON gaz.gaz01 = zr.zr01 AND gaz.gaz02 = '0'
WHERE  zr.zr02 = 'oea_file';   -- 表名(小寫)

-- 4) 某支程式畫面上的欄位標籤（原始欄位名稱 ↔ 畫面欄位名稱）
SELECT gae.gae02 原始欄位名稱, gae.gae04 畫面欄位名稱
FROM   ds.gae_file gae
WHERE  gae.gae01 LIKE 'axmt410%' AND gae.gae03 = '0';  -- 主畫面+子畫面"""

# ============================= Markdown =============================
md = []
A = md.append
A("# 鼎新 ERP（TIPTOP GP）測試站 Metadata：DS 字典 ↔ M2201 表/欄位 ↔ 畫面(程式) 整合分析")
A("")
A(f"> 產出日期：{TODAY}　|　資料來源：Oracle `toptest`（10.200.206.130:1521）　|　全程僅執行 `SELECT`")
A("> `GAT/GAQ/PAT` 以 `RO_M2201` 唯讀帳號查詢；`ZZ/GAZ/ZR/GAE` 該帳號無權限，以 `sys`（DB-INFO.md）唯讀查詢。")
A("")
A("## 0. 重要前提與資料來源")
A("")
A("| 項目 | 說明 |")
A("| --- | --- |")
A("| 資料庫版本 | Oracle Database 11g Release 11.2.0.3.0（thin JDBC 連線，ojdbc8）|")
A("| 字元集 | `AL32UTF8`（中文正確，無亂碼）|")
A("| 系統類型 | 鼎新 **TIPTOP GP** ERP；每家公司/帳套為獨立 schema，表結構幾乎相同 |")
A("| 資料字典 | 全域字典放在 `DS` schema：見第 3 節六張字典表（表名/欄位/程式/畫面欄位） |")
A("| 資料列數 | 一律取 **統計值 `ALL_TABLES.NUM_ROWS`**（非即時 `COUNT(*)`）|")
A("| 主要分析對象 | 公司帳套 **`M2201`**（其餘公司 schema 為相同表模型之複本，僅資料量不同）|")
A("")
A("> ⚠️ 區分事實與推導：標示「**統計值**」者來自 Oracle 統計資訊；標示「**推導/推測**」者為依命名慣例或 TIPTOP 慣例推斷，非實際查得。")
A("")
A("### 資訊來源標示（本文件所有欄位依此分類）")
A("")
A("| 標示 | 意義 | 來源 | 本文件中的欄位 |")
A("| --- | --- | --- | --- |")
A("| 【字典】 | 資料表/欄位層的字典**事實** | `GAT_FILE`／`GAQ_FILE`（直接查表） | 表中文名(GAT)、字典中文名(GAQ) |")
A("| 【DB】 | Oracle 資料庫本身的**事實** | `ALL_CONSTRAINTS`／`ALL_TAB_COLUMNS`／`NUM_ROWS` | 型別、可空、鍵(PK)、列數(統計值) |")
A("| 【作業】 | 作業(程式)層的**事實** | `ZZ_FILE`／`GAZ_FILE`／`ZR_FILE`（直接查表） | 程式代號、程式名稱(GAZ)、相關程式與異動別(I/U/D/S) |")
A("| 【畫面】 | 畫面層的**事實**：某畫面上某欄位的顯示名稱 | `GAE_FILE`（直接查表，依「畫面＋欄位」設定） | 全部畫面標籤、主要程式畫面標籤的「名稱」本身 |")
A("| 【推導】 | 我依規則推斷的**選擇/判斷**，可能個別出錯 | 演算法（非字典欄位） | 主要維護程式（挑哪支）、單頭/單身關聯、模組中文名 |")
A("")
A("> 注意「主要程式畫面標籤」是複合的：**名稱本身**是【畫面】事實（`GAE_FILE` 查得），但**以哪支程式為準**是【推導】。不想吃這個推導，就看「全部畫面標籤」欄。")
A("")
A("## 1. 總覽：各 Schema 角色與資料量")
A("")
A("| Schema | 角色（推測） | 表數 | 有資料表數 | 資料列數(統計值) | 統計日期 |")
A("| --- | --- | ---: | ---: | ---: | --- |")
for o in owners:
    A(f"| `{o['OWNER']}` | {ROLE.get(o['OWNER'],'—')} | {o['TABS']} | {o['TABS_WITH_DATA']} | {int(o['TOTAL_ROWS']):,} | {o['LAST_STAT']} |")
A("")
A("## 2. 已知條件驗證")
A("")
A("| # | 已知條件 | 驗證方法 | 結論 |")
A("| --- | --- | --- | --- |")
A("| 1 | 僅 `DS`、`M2201`、`MDF`、`PATCHTEMP`、`SDF` 有資料 | 以 `ALL_TABLES.NUM_ROWS` 統計各 owner 資料量 | **不成立（需修正）**。這 5 個 schema 確實有資料，但**並非只有它們**：`SDF`(5,919萬列)、`GDF`(4,937萬)、`DF`(1,263萬)、`HW`、`F2204`、`FT` 等多個公司帳套同樣有資料。`PATCHTEMP` 僅 1,498 列。 |")
A("| 2 | `DS.GAT_FILE.GAT01` 存放其他 schema 的 Table 名稱 | 直接查 `DS.GAT_FILE` 結構與內容 | **成立**。`GAT01`=表名（小寫）、`GAT02`=語言別（**0=繁體、2=简体**）、`GAT03`=**表中文名**、`GAT06`=模組代碼。共 4,857 列（繁體 2,430）。 |")
A("| 3 | 是否存在欄位字典表（GAT_ITEM 或類似） | 列舉 `DS` 之 `GA*` 系列並取樣 | **修正**：無 `GAT_ITEM`；欄位字典為 **`DS.GAQ_FILE`**：`GAQ01`=欄位名（小寫）、`GAQ02`=語言別、`GAQ03`=**欄位中文名**。繁體 54,196 列。`RO_M2201` 於 `DS` 僅被授權 `GAT_FILE`、`GAQ_FILE`、`PAT_FILE`。 |")
A("| 4 | 資料表是否有實體外鍵 | 查 `ALL_CONSTRAINTS` | **無任何實體 FK**（`M2201` 中 `R` 類型 = 0；僅 PK 2,145、檢查條件 10,858）→ 關聯以**命名慣例推導**。 |")
A("| 5 | TIPTOP 有「畫面檔」，可對應表與欄位 | 列舉 `DS` 字典表並取樣驗證 | **成立**。程式(畫面)主檔=`ZZ_FILE`、畫面中文名=`GAZ_FILE`、程式↔表關連=`ZR_FILE`、**畫面欄位名稱=`GAE_FILE`**。`GAE02` 相異值 50,267 個，其中 **32,423** 個可直接對回 `GAQ_FILE` 欄位字典（即真實 DB 欄位名），其餘為純 UI 元件（`cnt`、`page01`…）。`ZR03` 分佈：S=77,302 / I=8,607 / U=8,750 / D=7,603。 |")
A("")
A(f"> 字典覆蓋率（M2201 有資料的 {n_tbl} 張表）：欄位共 {fmt(n_col_total)} 個，`GAQ_FILE` 中文名覆蓋 {fmt(n_col_dict)}、`GAE_FILE` 畫面標籤覆蓋 {fmt(n_col_screen)}；{n_tbl_prog} 張表可由 `ZR_FILE` 對應到程式（{n_tbl_writer} 張有明確維護程式）。")
A("")
A("## 3. DS Schema 資料表總覽（字典表結論 + 全表清單）")
A("")
A("### 3.1 核心字典表（本文件對應鏈使用）")
A("")
A("| DS 字典表 | 角色 | 關鍵欄位 | 筆數 |")
A("| --- | --- | --- | ---: |")
for r in DICT_TABLES:
    A(f"| `{r[0]}` | {r[1]} | {r[2]} | {r[3]} |")
A("")
A("**對應鏈**：表 →（`ZR_FILE`）→ 程式/畫面 →（`GAZ_FILE` 取畫面中文名、`GAE_FILE` 取畫面欄位名稱）；表/欄位中文名則直接查 `GAT_FILE`/`GAQ_FILE`；選單結構查 `ZM_FILE`。")
A("")
A("```sql")
A(JOIN_SQL)
A("```")
A("")
ds_with_data = [t for t in ds_tabs if int(t["NUM_ROWS"] or 0) > 0]
ds_empty_n = len(ds_tabs) - len(ds_with_data)
A(f"### 3.2 DS 全部資料表清單（共 {len(ds_tabs)} 張；有資料 {len(ds_with_data)} 張，以下全列）")
A("")
A(f"> DS 是**系統/字典 schema**：除字典與系統設定表外，也含一套與公司帳套同構的業務表（多為空表；空表 {ds_empty_n} 張僅計數不列出）。「用途分類」為【推導】（依表名前綴與中文名關鍵字），其餘欄位為【字典】/【DB】事實。")
A("")
A("| # | 表名 | 中文名(GAT)【字典】 | 模組(GAT06)【字典】 | 列數(統計值)【DB】 | 用途分類【推導】 |")
A("| ---: | --- | --- | --- | ---: | --- |")
for i, t in enumerate(ds_with_data, 1):
    A(f"| {i} | `{t['TABLE_NAME']}` | {md_escape(t['ZH_NAME']) or '—'} | {t['MODULE'] or '—'} | {int(t['NUM_ROWS']):,} | {ds_category(t['TABLE_NAME'], t['ZH_NAME'])} |")
A("")
A("## 4. 關聯模型（單頭/單身，命名慣例推導）")
A("")
A("- **實體外鍵（FK）**：經 `ALL_CONSTRAINTS` 驗證為 **0 筆**（事實）。")
A(f"- **單頭/單身關聯（推導，中可信度）**：TIPTOP 以 3 碼前綴命名（表＝`<前綴>_file`，欄位＝`<前綴><序號>`）。同一前 2 碼的表屬同一單據族，字尾較前者（多為 `A`）為單頭/主檔，透過單據號欄位串接（如 `oeb01`=`oea01`）。共 {len(rels)} 條推導關聯、{len(families)} 個族。")
A("")
A("| 單頭/主檔 | 中文名 | 單身/明細（同族其餘表） |")
A("| --- | --- | --- |")
for h_ in sorted(fam_detail):
    A(f"| `{h_}` | {md_escape(table_zh_map.get(h_, '—'))} | {', '.join(f'`{d}`' for d in fam_detail[h_])} |")
A("")
A("> 註：上表為**命名慣例推導**，非字典或 FK 明示；個別表可能為獨立主檔而非單頭。")
A("")
A("### 4.2 GAU_FILE：鼎新字典層 PK/FK（【字典】事實，與 4.1 互補）")
A("")
A(f"`DS.GAU_FILE` 登錄了 **{len(gau):,} 條** 邏輯外鍵（`GAU01/02`=PK 欄位/表 ← `GAU03/04`=FK 欄位/表）。品質驗證：")
A("")
A(f"- 涉及 {len(gau_pk_tabs)} 張被參照(主檔)表、{len(gau_fk_tabs)} 張參照表；M2201 有資料的 333 張表中 **{gau_cover}** 張出現在其中。")
A(f"- 雙方都是 M2201 有資料表者 {len(gau_both)} 條，其中 **{gau_ok} 條欄位皆實際存在**（正確率 {gau_ok*100//max(1,len(gau_both))}%），品質可靠。")
A(f"- **性質是「欄位→主檔」的參照型 FK**（如各單據的料號→`ima_file`、科目→`aag_file`）：被參照最多者 " + "、".join(f"`{t}`({n})" for t, n in gau_top[:6]) + "。")
A(f"- **單頭/單身關聯幾乎沒登錄**：4.1 推導的 {len(rels)} 條中僅 **{gau_hit_derived}** 條出現在 GAU（連 `oea_file`←`oeb_file` 都沒有）→ GAU 與命名推導**互補**，不能取代。")
A("")
A("| 被參照主檔(PK) | 中文名 | 被參照次數 |")
A("| --- | --- | ---: |")
for tname_, n_ in gau_top:
    A(f"| `{tname_}` | {md_escape(zh_any.get(tname_, ''))} | {n_} |")
A("")
A("> 完整 1,957 條清單：HTML 版 4.2 可過濾檢視，或 `_data/ds_gau.tsv`。")
A("")
A("## 5. 資料表 ↔ 主要程式(畫面) 一覽（依列數排序）")
A("")
A("> 「主要維護程式」= 對該表有 I/U/D 動作、且其畫面(`GAE_FILE`)實際顯示本表欄位最多者（**推導**，非鼎新官方欄位）。")
A("")
A("| # | 資料表 | 表中文名(GAT)【字典】 | 模組【推導】 | 列數(統計值)【DB】 | 主要維護程式【推導】 | 程式名稱(GAZ)【作業】 | 相關程式數【作業】 |")
A("| ---: | --- | --- | --- | ---: | --- | --- | ---: |")
for i, t in enumerate(by_rows, 1):
    A(f"| {i} | `{t['table']}` | {md_escape(t['zh']) or '—'} | {mod_label(t['module'])} | {fmt(t['rows'])} | `{t['main_prog']}` | {t['main_prog_zh']} | {len(t['progs'])} |")
A("")
A("### 5.2 反查：程式(畫面) → 影響哪些資料表、做哪些操作【作業】")
A("")
A(f"> 來源 `ZR_FILE`（事實）。共 **{len(writer_progs)}** 支程式對 M2201 有資料表有寫入動作（I/U/D）。操作欄意義：I=新增(Insert)、U=更新(Update)、D=刪除(Delete)、S=查詢(Select)。")
A("> **欄位級影響需讀 4GL 原始碼**，字典僅到資料表層級，故本節以資料表為主；★ = 該程式是這張表的主要維護程式（推導）。")
A("> 下表為摘要（每程式一列）；逐表明細（含 I/U/D/S 打勾）見 HTML 版 5.2 或 `_data/m2201_table_programs.tsv`。")
A("")
A("| 程式 | 程式名稱(GAZ) | 模組 | 主要維護表(★) | 寫入表數 | 查詢表數 |")
A("| --- | --- | --- | --- | ---: | ---: |")
for p in writer_order:
    lst = writer_progs[p]
    mains = [x[0] for x in lst if x[3]]
    n_w = sum(1 for x in lst if x[2] & {"I", "U", "D"})
    n_s = sum(1 for x in lst if "S" in x[2])
    main_str = ", ".join(f"`{m}`" for m in mains[:6]) + ("…" if len(mains) > 6 else "")
    A(f"| `{p}` | {md_escape(prog_zh.get(p, ''))} | {prog_mod.get(p, '')} | {main_str} | {n_w} | {n_s} |")
A("")
A("## 6. Column 清單（依模組分組）：原始欄位 ↔ 字典中文名 ↔ 畫面欄位名稱")
A("")
A("> 「主要程式畫面標籤」= 主要維護程式畫面上的欄位名稱（`GAE_FILE`）；「全部畫面標籤」= 該欄位在全系統畫面上出現過的**所有**相異名稱，括號為出現畫面數（`GAE_FILE` 依「畫面＋欄位」設定，同一欄位在不同畫面可有不同名稱）。")
A("")
for m in order_mod:
    g = by_mod.get(m)
    if not g:
        continue
    A(f"### 模組 `{m or '(未分類)'}` — {mod_label(m)}")
    A("")
    for t in sorted(g, key=lambda x: (-x["rows"], x["table"])):
        prog_list = "、".join(f"`{p}`({acts_str(a)})" for p, a in t["progs"][:8]) + ("…" if len(t["progs"]) > 8 else "")
        A(f"#### `{t['table']}` — {md_escape(t['zh']) or '—'}（約 {fmt(t['rows'])} 列）")
        A("")
        A(f"- 主要維護程式【推導】：`{t['main_prog']}` {t['main_prog_zh']}　|　相關程式（前 8）【作業】：{prog_list or '（ZR_FILE 無對應）'}")
        A("")
        A("| 原始欄位 | 字典中文名(GAQ)【字典】 | 型別【DB】 | 可空【DB】 | 鍵【DB】 | 主要程式畫面標籤【畫面+推導】 | 全部畫面標籤（畫面數）【畫面】 |")
        A("| --- | --- | --- | :-: | :-: | --- | --- |")
        for c in t["cols"]:
            pk = "PK" if c["pk"] else ""
            labs = "、".join(f"{md_escape(lab)}({n})" for lab, n in c["labs"])
            A(f"| `{c['col']}` | {md_escape(c['dict_zh']) or '—'} | {c['type']} | {c['null']} | {pk} | {md_escape(c['main_lab'])} | {labs} |")
        A("")
A("## 7. 權限模型（RBAC）與改寫設計建議")
A("")
A("TIPTOP 的權限是標準 **RBAC + 資料範圍旗標** 模型，核心五張表都在 `DS`（以下結構與統計皆為查表**事實**）：")
A("")
A("### 7.1 授權鏈")
A("")
A("```")
A("員工 ZX_FILE ──ZXY_FILE──▶ 可進哪些營運中心(公司帳套)")
A("   │ ZX04=主要權限類別(角色)")
A("   ▼")
A("角色 ZW_FILE ──ZY_FILE──▶ 每支程式(畫面)允許的功能按鈕 + 資料範圍旗標")
A("   ▲")
A("   └─ ZXW_FILE：使用者層級的個別加/減權（覆寫角色）")
A("欄位級：GAJ_FILE（畫面元件對群組/個人 顯示/隱藏/可輸入）")
A("```")
A("")
A("### 7.2 各表結構（欄位中文名皆出自 GAQ 字典【字典】）")
A("")
PERM_DESC = {
    "ZX_FILE":  ("使用者主檔", "帳號/姓名/部門；`ZX04`=主要權限類別（**使用者直接掛一個角色**）；`ZX07/ZX08`=多營運中心/預設營運中心；`ZX16~ZX20`=密碼政策（更新日期、有效期、強制改密、試誤次數）"),
    "ZXY_FILE": ("員工↔營運中心授權", f"一人多筆＝可切多帳套；{zxy_users} 個使用者、{len(zxy_rows)} 筆"),
    "ZW_FILE":  ("權限類別（角色）主檔", f"`ZW01`=角色代號、`ZW02`=說明（如「【大豐】系統管理員」）、`ZW03`=角色綁定的選單目錄；共 {len(zw_rows)} 個角色"),
    "ZY_FILE":  ("角色×程式 功能授權（核心）", f"`ZY01`=角色、`ZY02`=程式代號、`ZY03`=允許功能（**逗號分隔字串**）、`ZY04/ZY05`=使用者/部門資料權限、`ZY07`=單身處理權限；{len(zy_rows):,} 筆、{zy_roles} 角色 × {zy_progs} 程式"),
    "ZXW_FILE": ("使用者個別權限覆寫", "`ZXW01`=使用者、`ZXW04`=權限類別、`ZXW05`=允許執行功能、`ZXW06/07/08`=資料所有人/部門/單身權限——同 ZY 結構但掛在個人"),
    "GAJ_FILE": ("畫面元件(欄位)級權限", "`GAJ01`=程式、`GAJ02`=欄位，對「群組/個人」設定 顯示/隱藏/可輸入（本站僅 1 筆，幾乎未使用）"),
}
for tn in ["ZX_FILE", "ZXY_FILE", "ZW_FILE", "ZY_FILE", "ZXW_FILE", "GAJ_FILE"]:
    role, note = PERM_DESC[tn]
    A(f"#### `DS.{tn}` — {role}")
    A("")
    A(note)
    A("")
    A("| 欄位 | 型別 | 中文名(GAQ) |")
    A("| --- | --- | --- |")
    for c in perm_cols_by_tab.get(tn, []):
        A(f"| `{c['COLUMN_NAME']}` | {c['DATA_TYPE']} | {md_escape(c['ZH']) or '—'} |")
    A("")
A("### 7.3 功能代碼與資料範圍（由 33,063 筆 ZY 實際統計【DB】）")
A("")
A("`ZY03` 的功能代碼就是畫面工具列的按鈕，最常出現的前 20 個：")
A("")
A("| 功能代碼 | 出現筆數 | 意義（推測） |")
A("| --- | ---: | --- |")
TOK_ZH = {"about":"關於","accept":"確定","cancel":"取消","exit":"離開","help":"求助","locale":"切換語言",
          "query":"查詢","insert":"新增","modify":"修改","delete":"刪除","first":"首筆","last":"末筆",
          "next":"下筆","previous":"上筆","jump":"跳至","detail":"單身明細","exporttoexcel":"匯出Excel",
          "confirm":"確認/核准","void":"作廢","undo_confirm":"取消確認","qbe_save":"儲存查詢條件",
          "qbe_select":"套用查詢條件","controlg":"畫面切換","controlz":"還原","controlp":"列印","output":"輸出"}
for tok, n in zy_tokens.most_common(20):
    A(f"| `{tok}` | {n:,} | {TOK_ZH.get(tok, '—')} |")
A("")
sc4 = "、".join(f"`{k or '(空)'}`={v:,}" for k, v in sorted(zy_scope4.items()))
sc5 = "、".join(f"`{k or '(空)'}`={v:,}" for k, v in sorted(zy_scope5.items()))
A(f"資料範圍旗標實際值域：`ZY04`(使用者資料權限) {sc4}；`ZY05`(部門資料權限) {sc5}。（實際為 **0~7 的分級值**而非單純開關；各級語意藏在 4GL 程式碼中，**未經官方文件證實**，改寫前建議先以實際單據比對各級行為）")
A("")
A("### 7.4 查詢範本：某使用者在某程式能做什麼")
A("")
A("```sql")
A("SELECT x.zx01 使用者, x.zx04 角色, y.zy02 程式, y.zy03 允許功能, y.zy04 資料權限, y.zy05 部門權限")
A("FROM   ds.zx_file x")
A("JOIN   ds.zy_file y ON y.zy01 = x.zx04")
A("WHERE  x.zx01 = '02501' AND y.zy02 = 'axmt410';")
A("-- 另需疊加 ds.zxw_file 的個人覆寫（同鍵 zxw01=使用者、zxw04 對應程式權限）")
A("```")
A("")
A("### 7.5 改寫設計建議【建議】")
A("")
A("若要在新系統重建這套權限，對應的現代化設計：")
A("")
A("| TIPTOP 現況 | 問題 | 建議設計 |")
A("| --- | --- | --- |")
A("| `ZY03` 功能清單存**逗號分隔字串** | 無法用 SQL 精確查「誰能刪除」；易 typo；無法加索引 | 正規化為 `role_permissions(role_id, program_id, action)` 一列一動作；action 建 enum/字典表 |")
A("| 使用者主檔直接掛**單一角色**（`ZX04`），個別差異靠 `ZXW` 覆寫 | 一人一角色，彈性差；覆寫邏輯分散兩張表 | 標準多對多 `user_roles`；個別加/減權改為明確的 allow/deny 覆寫表，解析順序 deny > user-allow > role |")
A("| 營運中心授權（`ZXY`）與功能授權（`ZY`）**互相獨立** | 無法表達「A 帳套是主管、B 帳套唯讀」 | 授權掛在 (user, tenant) 層級：`user_tenant_roles(user, tenant, role)` |")
A("| 資料範圍用旗標欄（`ZY04/05/07`：本人/部門/單身） | 語意藏在程式碼裡，值域無文件 | 明確的 scope 欄位（enum：`all / dept / own`）或 row-level policy，並寫進權限文件 |")
A("| 欄位級權限 `GAJ_FILE`（顯示/隱藏/可輸入） | 本站幾乎未用，但概念有價值 | UI 欄位權限矩陣 `field_permissions(role, program, field, visible/editable)`，僅對敏感欄位使用 |")
A("| 選單樹（`ZM`）與角色目錄（`ZW03`）**獨立維護** | 選單和權限可能不一致（看得到點不進去） | 選單由權限**推導**產生：能執行的程式才出現在選單 |")
A("| 密碼政策欄位在使用者主檔（`ZX16~20`） | 已具備雛形 | 移交 SSO/IdP（你們已有 DF-SSO），ERP 只留授權不留驗證 |")
A("")
A("## 8. 完成回報與限制")
A("")
A("- **資料列數**採統計值（帳號無法對多數表 `COUNT(*)`），統計過期者可能偏差。")
A("- **實體 FK 不存在**：表間關聯以命名慣例推導（中可信度）。另 `DS.GAU_FILE`（主鍵/外鍵關聯紀錄檔，1,957 列）可作補充來源，尚未展開。")
A("- **主要維護程式**為推導；同表可能被多支程式維護（完整清單見 `_data/m2201_table_programs.tsv`）。")
A(f"- **畫面標籤覆蓋 {fmt(n_col_screen)}/{fmt(n_col_total)}**：未覆蓋者多為純後端欄位（畫面本來不顯示）。")
A("- 其他公司 schema（SDF/GDF/DF…）與 M2201 同構，本文件字典可直接套用。")
A("- 機器可讀對照：`_data/m2201_table_programs.tsv`、`_data/m2201_column_screen_labels.tsv`、`_data/m2201_column_label_variants.tsv`（一列一名稱展開版）。")
A("")

# 9 英文語意名草稿(md 版僅列表層級 + 統計;欄位級 1.2 萬列詳見 html 第 9 章與 data/semantic_draft.tsv)
if semantic:
    n_sem_col = len(sem_cols)
    n_sem_src = collections.Counter(r.get("SRC", "") for r in sem_cols)
    A("## 9. 英文語意名草稿（`erp_metadata.semantic_mappings` 種子，待人工複核）")
    A("")
    A("> 全部 AI 產生之 **draft**：表名與欄位英文名(snake_case)依 GAQ/GAT 中文名翻譯,中文名缺漏者以 GAE 畫面標籤補,兩者皆無者保留原欄名。")
    A(f"> 覆蓋:表 {len(sem_tab)} 張、欄位 {fmt(n_sem_col)} 個(來源 GAQ {fmt(n_sem_src.get('GAQ',0))}、畫面標籤 {fmt(n_sem_src.get('GAE',0))}、原名保留 {fmt(n_sem_src.get('原名',0))})。")
    A("> 欄位級完整清單見 HTML 版第 9 章(可搜尋/分頁)與 `data/semantic_draft.tsv`;複核後匯入 RDS `erp_metadata.semantic_mappings`。")
    A("")
    A("| # | 表名 | 中文名(GAT) | 英文表名(草稿) |")
    A("| ---: | --- | --- | --- |")
    for i, t in enumerate(sorted(sem_tab.values(), key=lambda r: r["TABLE_NAME"]), 1):
        A(f"| {i} | `{t['TABLE_NAME']}` | {md_escape(t.get('ZH_NAME','') or '—')} | `{t['EN_NAME']}` |")
    A("")

md_text = "\n".join(md)
with open(os.path.join(OUT, "erp-metadata.md"), "w", encoding="utf-8") as f:
    f.write(md_text)
print("已寫出 docs/output/erp-metadata.md", len(md_text), "bytes")

# ============================= HTML =============================
def esc(s): return html.escape(str(s) if s is not None else "")

H = []
B = H.append
B("<!DOCTYPE html><html lang='zh-Hant'><head><meta charset='utf-8'>")
B("<meta name='viewport' content='width=device-width,initial-scale=1'>")
B("<title>TIPTOP GP Metadata：DS 字典 ↔ M2201 ↔ 畫面(程式) 整合分析</title>")
B("<style>")
# ---- 調色盤（CSS 變數，搬移專案時只改這裡）----
B(":root{--primary:#0f3d61;--primary-2:#1c4e80;--accent:#2e7dd1;--ink:#1a2233;--muted:#5b6b7d;"
  "--bg:#eef2f7;--card:#ffffff;--line:#e3e9f2;--line-strong:#d4dde9;--th-bg:#f0f5fb;--row-even:#f9fbfe;--row-hover:#eaf3fc;"
  "--warn-bg:#fff8e8;--warn-line:#e8a800;--code-bg:#edf1f7}")
B("*{box-sizing:border-box}")
B("body{font-family:'Segoe UI','Microsoft JhengHei',-apple-system,sans-serif;margin:0;color:var(--ink);background:var(--bg);font-size:17px;line-height:1.7}")
# ---- 左側 sidebar：標題 + meta + 垂直導覽（scroll-spy 高亮）----
B("aside#sb{position:fixed;left:0;top:0;bottom:0;width:264px;background:linear-gradient(180deg,#0b2f4d 0%,#0f3d61 60%,#123f66 100%);"
  "color:#fff;padding:26px 18px 20px;overflow-y:auto;z-index:10}")
B("aside#sb h1{margin:0 0 10px;font-size:20px;line-height:1.45;letter-spacing:.3px}")
B("aside#sb .meta{opacity:.72;font-size:13px;line-height:1.6;margin-bottom:18px;border-bottom:1px solid rgba(255,255,255,.15);padding-bottom:14px}")
B("aside#sb nav a{display:block;color:#cfe1f3;text-decoration:none;padding:8px 14px;border-radius:8px;margin:3px 0;font-size:15.5px;transition:background .15s,color .15s}")
B("aside#sb nav a:hover{background:rgba(255,255,255,.1);color:#fff}")
B("aside#sb nav a.on{background:#fff;color:var(--primary);font-weight:600}")
B("main{margin-left:264px;padding:26px 64px 90px}")
B("@media(max-width:1000px){aside#sb{position:static;width:auto;bottom:auto}main{margin-left:0;padding:24px 28px 60px}}")
# ---- 章節分頁：一次只顯示一章（sidebar = 換頁器）----
B("main>section{display:none}")
B("main>section.act{display:block}")
# ---- 清單分頁器 ----
B(".pgbar{display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin:12px 0}")
B("input.pgq{width:360px;max-width:100%;padding:8px 15px;border:1px solid var(--line-strong);border-radius:999px;font-size:15.5px;outline:none;transition:border .15s,box-shadow .15s;background:#fff}")
B("input.pgq:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(46,125,209,.15)}")
B(".pginfo{color:var(--muted);font-size:14.5px;white-space:nowrap}")
B(".pgnav{display:flex;gap:4px;flex-wrap:wrap}")
B(".pgnav button{border:1px solid var(--line-strong);background:#fff;color:var(--primary-2);min-width:36px;padding:4px 9px;border-radius:8px;font-size:14.5px;cursor:pointer;transition:background .12s}")
B(".pgnav button:hover:not(:disabled){background:#e4edf7}")
B(".pgnav button:disabled{opacity:.35;cursor:default}")
B(".pgnav button.cur{background:var(--primary);border-color:var(--primary);color:#fff;font-weight:600}")
# ---- 章節卡片 ----
B("section{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:26px 30px 30px;margin:26px 0;box-shadow:0 1px 3px rgba(15,61,97,.06)}")
B("h2{margin:0 0 14px;padding-bottom:10px;color:var(--primary);font-size:24px;border-bottom:1px solid var(--line);position:relative}")
B("h2::after{content:'';position:absolute;left:0;bottom:-1px;width:64px;height:3px;background:var(--accent);border-radius:2px}")
B("h3{margin-top:30px;color:var(--primary-2);font-size:20px}")
B("p{font-size:17px}")
# ---- 表格：sticky 表頭（需 border-collapse:separate，collapse 下邊框不會跟著黏住；top=nav 高 51px）----
B("table{border-collapse:separate;border-spacing:0;width:100%;font-size:16px;margin:14px 0;background:#fff;border:1px solid var(--line);border-radius:10px;overflow:hidden}")
B("th,td{border-bottom:1px solid var(--line);border-right:1px solid #eef2f8;padding:9px 13px;text-align:left;vertical-align:top}")
B("th:last-child,td:last-child{border-right:none}")
B("tr:last-child td{border-bottom:none}")
B("th{background:var(--th-bg);font-size:15px;color:var(--primary-2);position:sticky;top:0;z-index:5;white-space:nowrap}")
B("tbody tr:nth-child(even),tr:nth-child(even){background:var(--row-even)}")
B("tbody tr:hover,table tr:hover{background:var(--row-hover)}")
B("code{background:var(--code-bg);color:#12406b;padding:2px 7px;border-radius:5px;font-size:15px;font-family:Consolas,'Courier New',monospace}")
B(".warn{background:var(--warn-bg);border:1px solid #f3ddaa;border-left:5px solid var(--warn-line);border-radius:8px;padding:13px 17px;margin:14px 0;font-size:16px}")
# ---- 折疊卡片 ----
B("details{background:#fff;border:1px solid var(--line);border-radius:10px;margin:10px 0;padding:0 16px;transition:box-shadow .15s}")
B("details:hover{box-shadow:0 2px 8px rgba(15,61,97,.08)}")
B("details[open]{box-shadow:0 2px 10px rgba(15,61,97,.08)}")
B("summary{cursor:pointer;font-weight:600;color:var(--primary-2);font-size:17px;padding:11px 0;list-style-position:inside}")
B("details[open]>summary{border-bottom:1px dashed var(--line-strong);margin-bottom:8px}")
# 5.2 儲存格內的推導欄位小型展開（不吃卡片樣式）
B("details.cols{border:none;background:transparent;margin:0;padding:0;box-shadow:none}")
B("details.cols:hover,details.cols[open]{box-shadow:none}")
B("details.cols>summary{font-size:14.5px;font-weight:500;color:var(--accent);padding:0;border:none;margin:0}")
B("details.cols[open]>summary{border:none;margin-bottom:4px}")
B("details.cols{font-size:14px;color:var(--muted);line-height:1.6;font-family:Consolas,'Courier New',monospace}")
# ---- KPI 卡片 ----
B(".kpi{display:flex;gap:16px;flex-wrap:wrap;margin:18px 0}")
B(".kpi div{background:var(--card);border:1px solid var(--line);border-top:3px solid var(--accent);border-radius:10px;"
  "padding:13px 24px;min-width:150px;box-shadow:0 1px 3px rgba(15,61,97,.06);transition:transform .15s}")
B(".kpi div:hover{transform:translateY(-2px)}")
B(".kpi b{font-size:27px;color:var(--primary);display:block;letter-spacing:.5px}")
B("pre{background:#0b2f4d;color:#dcebfa;padding:16px 18px;border-radius:10px;overflow:auto;font-size:15px;line-height:1.6;font-family:Consolas,'Courier New',monospace}")
# ---- 過濾框 ----
B("input[id^='flt']{width:420px;max-width:100%;padding:9px 15px;border:1px solid var(--line-strong);border-radius:999px;font-size:16px;outline:none;transition:border .15s,box-shadow .15s;background:#fff}")
B("input[id^='flt']:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(46,125,209,.15)}")
# ---- 來源標示 badge ----
B(".b{display:inline-block;border-radius:999px;padding:0 9px;font-size:12.5px;line-height:21px;margin-left:6px;color:#fff;font-weight:500;white-space:nowrap;vertical-align:middle}")
B(".b-dict{background:#2563a8}.b-db{background:#5b6b7d}.b-prog{background:#2e8b57}.b-scr{background:#7c4dbe}.b-inf{background:#d97706}.b-sug{background:#0d9488}")
B("</style></head><body>")
B("<aside id='sb'><h1>鼎新 TIPTOP GP Metadata<br>DS 字典 ↔ M2201 ↔ 畫面(程式)</h1>")
B(f"<div class='meta'>產出日期 {TODAY}<br>Oracle toptest @10.200.206.130<br>全程唯讀，只下 SELECT<br>（GAT/GAQ 用 RO_M2201；其餘 DS 字典用 sys 唯讀）</div>")
B("<nav><a href='#legend'>資訊來源標示</a><a href='#ov'>1. 總覽</a><a href='#verify'>2. 已知條件驗證</a><a href='#dict'>3. DS 總覽</a>"
  "<a href='#rel'>4. 關聯模型</a><a href='#tp'>5. 表↔程式</a><a href='#cols'>6. 欄位對照</a><a href='#perm'>7. 權限模型</a><a href='#limit'>8. 限制</a>"
  "<a href='#sem'>9. 英文語意名草稿</a></nav></aside>")
B("<main>")
B("<div class='kpi'>")
B(f"<div><b>{n_tbl}</b>M2201 有資料表</div>")
B(f"<div><b>{fmt(n_col_total)}</b>欄位</div>")
B(f"<div><b>{fmt(n_col_dict)}</b>欄位有字典中文名</div>")
B(f"<div><b>{fmt(n_col_screen)}</b>欄位有畫面標籤</div>")
B(f"<div><b>{n_tbl_prog}</b>表可對應到程式</div>")
B(f"<div><b>{len(rels)}</b>推導關聯</div>")
B("</div>")

# 資訊來源圖例
B("<section id='legend'><h2>資訊來源標示（全文件通用）</h2>")
B("<table><tr><th>標示</th><th>意義</th><th>來源</th><th>本文件中的欄位</th></tr>")
LEGEND = [
 ("<span class='b b-dict'>字典</span>","資料表/欄位層的字典<b>事實</b>","<code>GAT_FILE</code>／<code>GAQ_FILE</code>（直接查表）","表中文名(GAT)、字典中文名(GAQ)"),
 ("<span class='b b-db'>DB</span>","Oracle 資料庫本身的<b>事實</b>","<code>ALL_CONSTRAINTS</code>／<code>ALL_TAB_COLUMNS</code>／<code>NUM_ROWS</code>","型別、可空、鍵(PK)、列數(統計值)"),
 ("<span class='b b-prog'>作業</span>","作業(程式)層的<b>事實</b>","<code>ZZ_FILE</code>／<code>GAZ_FILE</code>／<code>ZR_FILE</code>（直接查表）","程式代號、程式名稱(GAZ)、相關程式與異動別(I/U/D/S)"),
 ("<span class='b b-scr'>畫面</span>","畫面層的<b>事實</b>：某畫面上某欄位的顯示名稱","<code>GAE_FILE</code>（依「畫面＋欄位」設定）","全部畫面標籤、主要程式畫面標籤的「名稱」本身"),
 ("<span class='b b-inf'>推導</span>","依規則推斷的<b>選擇/判斷</b>，可能個別出錯","演算法（非字典欄位）","主要維護程式（挑哪支）、單頭/單身關聯、模組中文名"),
]
for r in LEGEND:
    B(f"<tr><td>{r[0]}</td><td>{r[1]}</td><td>{r[2]}</td><td>{r[3]}</td></tr>")
B("</table>")
B("<div class='warn'>「主要程式畫面標籤」是複合的：<b>名稱本身</b>是<span class='b b-scr'>畫面</span>事實（GAE_FILE 查得），"
  "但<b>以哪支程式為準</b>是<span class='b b-inf'>推導</span>。不想吃這個推導，請看「全部畫面標籤」欄。</div></section>")

# 1 總覽
B("<section id='ov'><h2>1. 總覽：各 Schema 角色與資料量</h2>")
B("<div class='warn'>系統為 <b>鼎新 TIPTOP GP</b>（Oracle 11g）。全域字典在 <code>DS</code> schema；"
  "每家公司/帳套一個 schema、表結構同構。資料列數為 <b>統計值</b>（<code>ALL_TABLES.NUM_ROWS</code>），非即時 COUNT。</div>")
B("<table><tr><th>Schema</th><th>角色（推測）</th><th>表數</th><th>有資料表數</th><th>資料列數(統計值)</th><th>統計日期</th></tr>")
for o in owners:
    B(f"<tr><td><code>{esc(o['OWNER'])}</code></td><td>{ROLE.get(o['OWNER'],'—').replace('**','')}</td>"
      f"<td style='text-align:right'>{o['TABS']}</td><td style='text-align:right'>{o['TABS_WITH_DATA']}</td>"
      f"<td style='text-align:right'>{int(o['TOTAL_ROWS']):,}</td><td>{esc(o['LAST_STAT'])}</td></tr>")
B("</table></section>")

# 2 已知條件驗證
B("<section id='verify'><h2>2. 已知條件驗證</h2>")
B("<table><tr><th>#</th><th>已知條件</th><th>驗證方法</th><th>結論</th></tr>")
VERIFY = [
 ("1","僅 DS、M2201、MDF、PATCHTEMP、SDF 有資料","ALL_TABLES.NUM_ROWS 統計各 owner",
  "<b>不成立（需修正）</b>。這 5 個 schema 有資料，但並非只有它們：SDF(5,919萬)、GDF(4,937萬)、DF(1,263萬)、HW、F2204、FT 等公司帳套同樣有資料。"),
 ("2","DS.GAT_FILE.GAT01 存放其他 schema 的表名","直接查 DS.GAT_FILE",
  "<b>成立</b>。GAT01=表名(小寫)、GAT02=語言別(<b>0=繁體、2=简体</b>)、GAT03=<b>表中文名</b>、GAT06=模組。共 4,857 列（繁體 2,430）。"),
 ("3","是否存在欄位字典表（GAT_ITEM 或類似）","列舉 DS 之 GA* 系列並取樣",
  "<b>修正</b>：無 GAT_ITEM；欄位字典為 <b>DS.GAQ_FILE</b>（GAQ01=欄位名、GAQ03=<b>欄位中文名</b>）。繁體 54,196 列。RO_M2201 於 DS 僅被授權 GAT/GAQ/PAT。"),
 ("4","資料表是否有實體外鍵","查 ALL_CONSTRAINTS",
  "<b>無任何實體 FK</b>（M2201 中 R 類型=0）→ 關聯以命名慣例推導。"),
 ("5","TIPTOP 有「畫面檔」，可對應表與欄位","列舉 DS 字典表並取樣驗證",
  "<b>成立</b>。程式主檔=<b>ZZ_FILE</b>、畫面中文名=<b>GAZ_FILE</b>、程式↔表關連=<b>ZR_FILE</b>、畫面欄位名稱=<b>GAE_FILE</b>。"
  "GAE02 相異值 50,267，其中 <b>32,423</b> 可直接對回 GAQ 欄位字典（真實 DB 欄位名）；ZR03 分佈 S=77,302 / I=8,607 / U=8,750 / D=7,603。"),
]
for r in VERIFY:
    B(f"<tr><td>{r[0]}</td><td>{r[1]}</td><td><code>{r[2]}</code></td><td>{r[3]}</td></tr>")
B("</table>")
B(f"<div class='warn'>字典覆蓋率（M2201 有資料 {n_tbl} 張表）：欄位 {fmt(n_col_total)} 個，GAQ 中文名覆蓋 <b>{fmt(n_col_dict)}</b>、"
  f"GAE 畫面標籤覆蓋 <b>{fmt(n_col_screen)}</b>；{n_tbl_prog} 張表可對應到程式（{n_tbl_writer} 張有明確維護程式）。</div></section>")

# 3 DS schema 總覽
B("<section id='dict'><h2>3. DS Schema 資料表總覽（字典表結論 + 全表清單）</h2>")
B("<h3>3.1 核心字典表（本文件對應鏈使用）</h3>")
B("<table><tr><th>DS 字典表</th><th>角色</th><th>關鍵欄位</th><th>筆數</th></tr>")
for r in DICT_TABLES:
    B(f"<tr><td><code>{r[0]}</code></td><td>{r[1].replace('**','')}</td><td>{esc(r[2]).replace('**','')}</td><td style='text-align:right'>{r[3]}</td></tr>")
B("</table>")
B("<p><b>對應鏈</b>：表 →（ZR_FILE）→ 程式/畫面 →（GAZ_FILE 取畫面中文名、GAE_FILE 取畫面欄位名稱）；表/欄位中文名直接查 GAT_FILE / GAQ_FILE；選單結構查 ZM_FILE。</p>")
B(f"<pre>{esc(JOIN_SQL)}</pre>")
B(f"<h3>3.2 DS 全部資料表清單（共 {len(ds_tabs)} 張；有資料 {len(ds_with_data)} 張，以下全列）</h3>")
B(f"<div class='warn'>DS 是<b>系統/字典 schema</b>：除字典與系統設定表外，也含一套與公司帳套同構的業務表（多為空表；空表 {ds_empty_n} 張僅計數不列出）。"
  "「用途分類」為<span class='b b-inf'>推導</span>（依表名前綴與中文名關鍵字），其餘欄位為<span class='b b-dict'>字典</span>/<span class='b b-db'>DB</span>事實。</div>")
PGBAR = "<div class='pgbar'><input class='pgq' placeholder='{ph}'><span class='pginfo'></span><span class='pgnav'></span></div>"
B("<div class='pgwrap' data-size='50' data-list='tbody'>")
B(PGBAR.format(ph="搜尋表名 / 中文名 / 模組 / 分類"))
B("<table><thead><tr><th>#</th><th>表名</th><th>中文名(GAT)<span class='b b-dict'>字典</span></th><th>模組(GAT06)<span class='b b-dict'>字典</span></th>"
  "<th>列數(統計值)<span class='b b-db'>DB</span></th><th>用途分類<span class='b b-inf'>推導</span></th></tr></thead><tbody>")
for i, t in enumerate(ds_with_data, 1):
    B(f"<tr><td style='text-align:right'>{i}</td><td><code>{esc(t['TABLE_NAME'])}</code></td><td>{esc(t['ZH_NAME'])}</td>"
      f"<td>{esc(t['MODULE'] or '')}</td><td style='text-align:right'>{int(t['NUM_ROWS']):,}</td>"
      f"<td>{esc(ds_category(t['TABLE_NAME'], t['ZH_NAME']))}</td></tr>")
B("</tbody></table>")
B(PGBAR.format(ph="搜尋表名 / 中文名 / 模組 / 分類"))
B("</div></section>")

# 4 關聯
B("<section id='rel'><h2>4. 關聯模型（單頭/單身，命名慣例推導）</h2>")
B(f"<div class='warn'>實體外鍵 <b>0 筆</b>（事實）。單頭/單身以 3 碼前綴推導：同前 2 碼為同一單據族，"
  f"字尾 A 多為單頭/主檔，透過單據號欄位串接（如 <code>oeb01</code>=<code>oea01</code>）。共 {len(rels)} 條、{len(families)} 個族。</div>")
B("<table><tr><th>單頭/主檔</th><th>中文名</th><th>單身/明細（同族其餘表）</th></tr>")
for h_ in sorted(fam_detail):
    B(f"<tr><td><code>{h_}</code></td><td>{esc(table_zh_map.get(h_,'—'))}</td><td>{', '.join(f'<code>{d}</code>' for d in fam_detail[h_])}</td></tr>")
B("</table>")
B("<h3>4.2 GAU_FILE：鼎新字典層 PK/FK<span class='b b-dict'>字典</span>（與 4.1 互補）</h3>")
B(f"<div class='warn'><code>DS.GAU_FILE</code> 登錄 <b>{len(gau):,} 條</b>邏輯外鍵。品質驗證：雙方皆為 M2201 有資料表者 {len(gau_both)} 條、"
  f"欄位皆實際存在 <b>{gau_ok}</b> 條（{gau_ok*100//max(1,len(gau_both))}%），品質可靠；333 張有資料表中 {gau_cover} 張出現在其中。"
  f"性質是<b>「欄位→主檔」參照型 FK</b>（料號→ima、科目→aag…）；<b>單頭/單身關聯幾乎沒登錄</b>"
  f"（4.1 推導 {len(rels)} 條僅 {gau_hit_derived} 條在 GAU，連 oea←oeb 都沒有）→ 與命名推導互補，不能互相取代。</div>")
B("<div class='pgwrap' data-size='50' data-list='tbody'>")
B(PGBAR.format(ph="搜尋表名 / 欄位 / 中文名，例如 ima_file 或 客戶"))
B("<table><thead><tr><th>參照表(FK)</th><th>中文名</th><th>FK 欄位</th><th>→</th><th>被參照主檔(PK)</th><th>中文名</th><th>PK 欄位</th></tr></thead><tbody>")
for r in sorted(gau, key=lambda x: (x["GAU02"], x["GAU04"], x["GAU03"])):
    B(f"<tr><td><code>{esc(r['GAU04'])}</code></td><td>{esc(zh_any.get(r['GAU04'], ''))}</td><td><code>{esc(r['GAU03'])}</code></td>"
      f"<td>→</td><td><code>{esc(r['GAU02'])}</code></td><td>{esc(zh_any.get(r['GAU02'], ''))}</td><td><code>{esc(r['GAU01'])}</code></td></tr>")
B("</tbody></table>")
B(PGBAR.format(ph="搜尋表名 / 欄位 / 中文名"))
B("</div></section>")

# 5 表↔程式
B("<section id='tp'><h2>5. 資料表 ↔ 主要程式(畫面) 一覽（依列數排序）</h2>")
B("<div class='warn'>「主要維護程式」= 對該表有 I/U/D 動作、且畫面（GAE_FILE）實際顯示本表欄位最多者（<b>推導</b>，非鼎新官方欄位）。完整表↔程式清單見 <code>_data/m2201_table_programs.tsv</code>。</div>")
B("<div class='pgwrap' data-size='50' data-list='tbody'>")
B(PGBAR.format(ph="搜尋表名 / 中文名 / 模組 / 程式代號，例如 oea、訂單、axmt410"))
B("<table><thead><tr><th>#</th><th>資料表</th><th>表中文名(GAT)<span class='b b-dict'>字典</span></th><th>模組<span class='b b-inf'>推導</span></th>"
  "<th>列數(統計值)<span class='b b-db'>DB</span></th><th>主要維護程式<span class='b b-inf'>推導</span></th>"
  "<th>程式名稱(GAZ)<span class='b b-prog'>作業</span></th><th>相關程式數<span class='b b-prog'>作業</span></th></tr></thead><tbody>")
for i, t in enumerate(by_rows, 1):
    B(f"<tr><td style='text-align:right'>{i}</td><td><code>{esc(t['table'])}</code></td><td>{esc(t['zh'])}</td>"
      f"<td>{esc(mod_label(t['module']))}</td><td style='text-align:right'>{fmt(t['rows'])}</td>"
      f"<td><code>{esc(t['main_prog'])}</code></td><td>{esc(t['main_prog_zh'])}</td>"
      f"<td style='text-align:right'>{len(t['progs'])}</td></tr>")
B("</tbody></table>")
B(PGBAR.format(ph="搜尋表名 / 中文名 / 模組 / 程式代號"))
B("</div>")
B("<h3 id='pt'>5.2 反查：程式(畫面) → 影響哪些資料表、做哪些操作<span class='b b-prog'>作業</span></h3>")
B(f"<div class='warn'>來源 <code>ZR_FILE</code>（事實）。共 <b>{len(writer_progs)}</b> 支程式對 M2201 有資料表有寫入動作（I/U/D）。"
  "★ = 該程式是這張表的主要維護程式（<span class='b b-inf'>推導</span>）。<br>"
  "有寫入的表另附「<b>推導欄位</b>」<span class='b b-inf'>推導</span>＝該表欄位 ∩ 該程式畫面(GAE_FILE)顯示的欄位，即這支程式實際操作的欄位集合。"
  "邊界：畫面顯示 ≠ 必定寫入（可能唯讀帶出）；程式在背景自動寫的欄位（審計欄、狀態碼）不會顯示在畫面上，故此清單是<b>下限</b>。欄位級的確切寫入仍需 4GL 原始碼。</div>")
B("<div class='pgwrap' data-size='30'>")
B(PGBAR.format(ph="搜尋程式代號 / 名稱 / 表名，例如 axmt410、訂單、oea"))
B("<div class='pglist'>")
for p in writer_order:
    lst = writer_progs[p]
    mains = [x[0] for x in lst if x[3]]
    n_w = sum(1 for x in lst if x[2] & {"I", "U", "D"})
    n_s = sum(1 for x in lst if "S" in x[2])
    key = esc((p + " " + prog_zh.get(p, "") + " " + " ".join(x[0].lower() for x in lst)).lower())
    star = f"　主要維護 {len(mains)} 表" if mains else ""
    B(f"<details data-k='{key}'><summary><code>{esc(p)}</code> — {esc(prog_zh.get(p, '（字典無名稱）'))}"
      f"　<span style='color:var(--muted);font-weight:400'>模組 {esc(prog_mod.get(p, '—'))}｜寫入 {n_w} 表｜查詢 {n_s} 表{star}</span></summary>")
    B("<table><thead><tr><th>資料表</th><th>中文名(GAT)</th><th>新增 I</th><th>更新 U</th><th>刪除 D</th><th>查詢 S</th><th>主要程式</th><th>推導欄位（畫面∩表）</th></tr></thead><tbody>")
    pf = prog_fields(p)
    for tbl, zh_, acts, is_main in sorted(lst, key=lambda x: (not x[3], not (x[2] & {"I","U","D"}), x[0])):
        def ck(flag): return "✓" if flag in acts else ""
        colcell = ""
        if acts & {"I", "U", "D"}:
            hit = [c for c in tbl_cols_l.get(tbl, []) if c in pf]
            if hit:
                colcell = (f"<details class='cols'><summary>{len(hit)}/{len(tbl_cols_l.get(tbl, []))} 欄</summary>"
                           + "、".join(hit) + "</details>")
        B(f"<tr><td><code>{esc(tbl)}</code></td><td>{esc(zh_)}</td>"
          f"<td style='text-align:center'>{ck('I')}</td><td style='text-align:center'>{ck('U')}</td>"
          f"<td style='text-align:center'>{ck('D')}</td><td style='text-align:center'>{ck('S')}</td>"
          f"<td style='text-align:center'>{'★' if is_main else ''}</td><td>{colcell}</td></tr>")
    B("</tbody></table></details>")
B("</div>")
B(PGBAR.format(ph="搜尋程式代號 / 名稱 / 表名"))
B("</div></section>")

# 6 欄位對照
B("<section id='cols'><h2>6. Column 清單：原始欄位 ↔ 字典中文名(GAQ) ↔ 畫面欄位名稱(GAE)</h2>")
B("<div class='pgwrap' data-size='20'>")
B(PGBAR.format(ph="搜尋表名 / 中文名 / 模組 / 程式代號，例如 oea、訂單、axmt410"))
B("<div class='pglist'>")
for t in by_rows:
    key = esc((t["table"] + " " + (t["zh"] or "") + " " + (t["module"] or "") + " " + mod_label(t["module"])
               + " " + t["main_prog"] + " " + t["main_prog_zh"]).lower())
    prog_list = "、".join(f"<code>{esc(p)}</code>({acts_str(a)})" for p, a in t["progs"][:8]) + ("…" if len(t["progs"]) > 8 else "")
    B(f"<details data-k='{key}'><summary><code>{esc(t['table'])}</code> — {esc(t['zh'] or '（無中文名）')}　"
      f"<span style='color:var(--muted);font-weight:400'>模組 {esc(t['module'] or '—')}</span>"
      f"｜主要程式<span class='b b-inf'>推導</span> <code>{esc(t['main_prog'])}</code> {esc(t['main_prog_zh'])}（約 {fmt(t['rows'])} 列）</summary>")
    if prog_list:
        B(f"<p>相關程式（前 8）<span class='b b-prog'>作業</span>：{prog_list}</p>")
    B("<table><tr><th>原始欄位</th><th>字典中文名(GAQ)<span class='b b-dict'>字典</span></th><th>型別<span class='b b-db'>DB</span></th>"
      "<th>可空<span class='b b-db'>DB</span></th><th>鍵<span class='b b-db'>DB</span></th>"
      "<th>主要程式畫面標籤<span class='b b-scr'>畫面</span><span class='b b-inf'>推導</span></th>"
      "<th>全部畫面標籤（畫面數）<span class='b b-scr'>畫面</span></th></tr>")
    for c in t["cols"]:
        pk = "PK" if c["pk"] else ""
        labs = "、".join(f"{esc(lab)}({n})" for lab, n in c["labs"])
        B(f"<tr><td><code>{esc(c['col'])}</code></td><td>{esc(c['dict_zh'])}</td><td>{esc(c['type'])}</td>"
          f"<td style='text-align:center'>{c['null']}</td><td style='text-align:center'>{pk}</td>"
          f"<td>{esc(c['main_lab'])}</td><td>{labs}</td></tr>")
    B("</table></details>")
B("</div>")
B(PGBAR.format(ph="搜尋表名 / 中文名 / 模組 / 程式代號"))
B("</div></section>")

# 7 權限模型
B("<section id='perm'><h2>7. 權限模型（RBAC）與改寫設計建議</h2>")
B("<div class='warn'>TIPTOP 權限 = 標準 <b>RBAC + 資料範圍旗標</b>。以下結構與統計為查表<b>事實</b>；7.5 為改寫<span class='b b-sug'>建議</span>。</div>")
B("<h3>7.1 授權鏈</h3>")
B("<pre>員工 ZX_FILE ──ZXY_FILE──▶ 可進哪些營運中心(公司帳套)\n   │ ZX04=主要權限類別(角色)\n   ▼\n角色 ZW_FILE ──ZY_FILE──▶ 每支程式(畫面)允許的功能按鈕 + 資料範圍旗標\n   ▲\n   └─ ZXW_FILE：使用者層級的個別加/減權（覆寫角色）\n欄位級：GAJ_FILE（畫面元件對群組/個人 顯示/隱藏/可輸入）</pre>")
B("<h3>7.2 各表結構（欄位中文名皆出自 GAQ 字典<span class='b b-dict'>字典</span>）</h3>")
for tn in ["ZX_FILE", "ZXY_FILE", "ZW_FILE", "ZY_FILE", "ZXW_FILE", "GAJ_FILE"]:
    role, note = PERM_DESC[tn]
    B(f"<details><summary><code>DS.{tn}</code> — {esc(role)}</summary>")
    B(f"<p>{note.replace('**','').replace('`','')}</p>")
    B("<table><tr><th>欄位</th><th>型別</th><th>中文名(GAQ)</th></tr>")
    for c in perm_cols_by_tab.get(tn, []):
        B(f"<tr><td><code>{esc(c['COLUMN_NAME'])}</code></td><td>{esc(c['DATA_TYPE'])}</td><td>{esc(c['ZH'])}</td></tr>")
    B("</table></details>")
B(f"<h3>7.3 功能代碼與資料範圍（由 {len(zy_rows):,} 筆 ZY 實際統計<span class='b b-db'>DB</span>）</h3>")
B("<p><code>ZY03</code> 的功能代碼就是畫面工具列的按鈕，最常出現的前 20 個：</p>")
B("<table><tr><th>功能代碼</th><th>出現筆數</th><th>意義（推測）</th></tr>")
for tok, n in zy_tokens.most_common(20):
    B(f"<tr><td><code>{esc(tok)}</code></td><td style='text-align:right'>{n:,}</td><td>{esc(TOK_ZH.get(tok, '—'))}</td></tr>")
B("</table>")
B(f"<p>資料範圍旗標實際值域：<code>ZY04</code>(使用者資料權限) {esc(sc4)}；<code>ZY05</code>(部門資料權限) {esc(sc5)}。"
  "（實際為 <b>0~7 的分級值</b>而非單純開關；各級語意藏在 4GL 程式碼中，<b>未經官方文件證實</b>，改寫前建議先以實際單據比對各級行為）</p>")
B("<h3>7.4 查詢範本：某使用者在某程式能做什麼</h3>")
B("<pre>SELECT x.zx01 使用者, x.zx04 角色, y.zy02 程式, y.zy03 允許功能, y.zy04 資料權限, y.zy05 部門權限\nFROM   ds.zx_file x\nJOIN   ds.zy_file y ON y.zy01 = x.zx04\nWHERE  x.zx01 = '02501' AND y.zy02 = 'axmt410';\n-- 另需疊加 ds.zxw_file 的個人覆寫</pre>")
B("<h3>7.5 改寫設計建議 <span class='b b-sug'>建議</span></h3>")
B("<table><tr><th>TIPTOP 現況</th><th>問題</th><th>建議設計</th></tr>")
SUGGEST = [
 ("ZY03 功能清單存<b>逗號分隔字串</b>","無法用 SQL 精確查「誰能刪除」；易 typo；無法加索引","正規化為 <code>role_permissions(role_id, program_id, action)</code> 一列一動作；action 建 enum/字典表"),
 ("使用者主檔直接掛<b>單一角色</b>（ZX04），個別差異靠 ZXW 覆寫","一人一角色，彈性差；覆寫邏輯分散兩張表","標準多對多 <code>user_roles</code>；個別加/減權改為明確的 allow/deny 覆寫表，解析順序 deny &gt; user-allow &gt; role"),
 ("營運中心授權（ZXY）與功能授權（ZY）<b>互相獨立</b>","無法表達「A 帳套是主管、B 帳套唯讀」","授權掛在 (user, tenant) 層級：<code>user_tenant_roles(user, tenant, role)</code>"),
 ("資料範圍用旗標欄（ZY04/05/07：本人/部門/單身）","語意藏在程式碼裡，值域無文件","明確的 scope 欄位（enum：<code>all / dept / own</code>）或 row-level policy，並寫進權限文件"),
 ("欄位級權限 GAJ_FILE（顯示/隱藏/可輸入）","本站幾乎未用，但概念有價值","UI 欄位權限矩陣 <code>field_permissions(role, program, field, visible/editable)</code>，僅對敏感欄位使用"),
 ("選單樹（ZM）與角色目錄（ZW03）<b>獨立維護</b>","選單和權限可能不一致（看得到點不進去）","選單由權限<b>推導</b>產生：能執行的程式才出現在選單"),
 ("密碼政策欄位在使用者主檔（ZX16~20）","已具備雛形","移交 SSO/IdP（已有 DF-SSO），ERP 只留授權不留驗證"),
]
for r in SUGGEST:
    B(f"<tr><td>{r[0]}</td><td>{r[1]}</td><td>{r[2]}</td></tr>")
B("</table></section>")

# 8 限制
B("<section id='limit'><h2>8. 完成回報與限制</h2><ul>")
B("<li>資料列數採<b>統計值</b>，統計過期者可能偏差。</li>")
B("<li>實體 FK 不存在，關聯以命名慣例推導（中可信度）；<code>DS.GAU_FILE</code>（主鍵/外鍵關聯紀錄檔，1,957 列）可作補充來源，尚未展開。</li>")
B("<li>「主要維護程式」為推導；同表可能被多支程式維護（完整清單見 <code>_data/m2201_table_programs.tsv</code>）。</li>")
B(f"<li>畫面標籤覆蓋 {fmt(n_col_screen)}/{fmt(n_col_total)} 欄位；未覆蓋者多為純後端欄位（畫面本來不顯示）。</li>")
B("<li>其他公司 schema（SDF/GDF/DF…）與 M2201 同構，字典可直接套用。</li>")
B("<li>機器可讀對照：<code>_data/m2201_table_programs.tsv</code>、<code>_data/m2201_column_screen_labels.tsv</code>、<code>_data/m2201_column_label_variants.tsv</code>（一列一名稱展開版）。</li>")
B("</ul></section>")

# 9 英文語意名草稿(semantic_mappings 種子)
B("<section id='sem'><h2>9. 英文語意名草稿（<code>erp_metadata.semantic_mappings</code> 種子）</h2>")
if semantic:
    n_sem_col = len(sem_cols)
    n_sem_src = collections.Counter(r.get("SRC", "") for r in sem_cols)
    B("<div class='warn'>本章全部為 <b>AI 產生之 draft</b>,供人工複核後匯入 RDS <code>erp_metadata.semantic_mappings</code>(status=draft→confirmed)。"
      "命名規則:小寫 snake_case;表名=語意化複數(單頭檔→<code>_headers</code>、單身檔→<code>_details</code>、記錄檔→<code>_logs</code>),"
      "<b>GAT 字典無中文名的表保留原表名小寫</b>(如 <code>TC_IMA_FILE</code>→<code>tc_ima_file</code>);"
      "欄位依 GAQ 中文名翻譯,GAQ 缺者以 GAE 畫面標籤補<span class='b b-scr'>畫面</span>,兩者皆無者保留原欄名。"
      "mapping 全域一份、不分帳套 schema(帳套同構,同 DS 字典設計)。</div>")
    B("<div class='kpi'>")
    B(f"<div><b>{len(sem_tab)}</b>表名已翻譯</div>")
    B(f"<div><b>{fmt(n_sem_col)}</b>欄位已翻譯</div>")
    B(f"<div><b>{fmt(n_sem_src.get('GAQ', 0))}</b>依 GAQ 中文名</div>")
    B(f"<div><b>{fmt(n_sem_src.get('GAE', 0))}</b>依畫面標籤</div>")
    B(f"<div><b>{fmt(n_sem_src.get('原名', 0))}</b>無來源保留原名</div>")
    B("</div>")
    B("<h3>9.1 表名對照（表層級映射,<code>column_name=''</code>）</h3>")
    B("<div class='pgwrap' data-size='50' data-list='tbody'>")
    B(PGBAR.format(ph="搜尋表名 / 中文名 / 英文名,例如 oea、訂單、order"))
    B("<table><thead><tr><th>#</th><th>表名</th><th>中文名(GAT)<span class='b b-dict'>字典</span></th><th>模組</th><th>英文表名(草稿)<span class='b b-sug'>建議</span></th></tr></thead><tbody>")
    _sem_mod = {t["TABLE_NAME"]: t.get("MODULE", "") for t in tables}
    for i, t in enumerate(sorted(sem_tab.values(), key=lambda r: r["TABLE_NAME"]), 1):
        B(f"<tr><td style='text-align:right'>{i}</td><td><code>{esc(t['TABLE_NAME'])}</code></td><td>{esc(t.get('ZH_NAME','') or '—')}</td>"
          f"<td>{esc(_sem_mod.get(t['TABLE_NAME'], ''))}</td><td><code>{esc(t['EN_NAME'])}</code></td></tr>")
    B("</tbody></table>")
    B(PGBAR.format(ph="搜尋表名 / 中文名 / 英文名"))
    B("</div>")
    B("<h3>9.2 欄位對照（全部欄位,可搜尋）</h3>")
    B("<div class='pgwrap' data-size='100' data-list='tbody'>")
    B(PGBAR.format(ph="搜尋表名 / 欄位 / 中文名 / 英文名,例如 gen01、員工、employee"))
    B("<table><thead><tr><th>資料表</th><th>欄位</th><th>中文名<span class='b b-dict'>字典</span></th><th>英文欄名(草稿)<span class='b b-sug'>建議</span></th><th>來源</th></tr></thead><tbody>")
    SRC_BADGE = {"GAQ": "<span class='b b-dict'>GAQ</span>", "GAE": "<span class='b b-scr'>畫面</span>", "原名": "<span class='b b-inf'>原名</span>"}
    for r in sem_cols:
        B(f"<tr><td><code>{esc(r['TABLE_NAME'])}</code></td><td><code>{esc(r['COLUMN_NAME'])}</code></td><td>{esc(r.get('ZH_NAME','') or '—')}</td>"
          f"<td><code>{esc(r['EN_NAME'])}</code></td><td>{SRC_BADGE.get(r.get('SRC',''), esc(r.get('SRC','')))}</td></tr>")
    B("</tbody></table>")
    B(PGBAR.format(ph="搜尋表名 / 欄位 / 中文名 / 英文名"))
    B("</div>")
else:
    B("<div class='warn'>尚未產生 <code>data/semantic_draft.tsv</code>(翻譯批次未合併);合併後重跑本產生器即出現內容。</div>")
B("</section></main>")
# 章節換頁路由（sidebar = 分頁器，一次只顯示一章）+ 清單分頁元件
B("""<script>
(function(){
  // ---- 章節分頁 ----
  const secs=[...document.querySelectorAll('main>section[id]')];
  const links={};document.querySelectorAll('aside#sb nav a[href^="#"]').forEach(a=>links[a.getAttribute('href').slice(1)]=a);
  function go(id){
    if(!secs.some(s=>s.id===id)) id=secs[0].id;
    secs.forEach(s=>s.classList.toggle('act',s.id===id));
    for(const k in links) links[k].classList.toggle('on',k===id);
    window.scrollTo(0,0);
  }
  window.addEventListener('hashchange',()=>go(location.hash.slice(1)));
  go(location.hash.slice(1));

  // ---- 清單分頁元件（搜尋跨全部項目，分頁只顯示當頁）----
  document.querySelectorAll('.pgwrap').forEach(w=>{
    const list=w.querySelector(w.dataset.list||'.pglist'); if(!list) return;
    const items=[...list.children], size=+w.dataset.size||50;
    const qs=[...w.querySelectorAll('input.pgq')];
    const infos=w.querySelectorAll('.pginfo'), navs=w.querySelectorAll('.pgnav');
    let page=1;
    const key=el=>(el.dataset.k||el.textContent).toLowerCase();
    function apply(){
      const v=(qs[0]?qs[0].value:'').trim().toLowerCase();
      const f=v?items.filter(el=>key(el).includes(v)):items;
      const pages=Math.max(1,Math.ceil(f.length/size));
      if(page>pages) page=pages;
      items.forEach(el=>el.style.display='none');
      f.slice((page-1)*size,page*size).forEach(el=>el.style.display='');
      infos.forEach(i=>i.textContent=`共 ${f.length.toLocaleString()} 項・第 ${page}/${pages} 頁`);
      navs.forEach(nv=>{nv.innerHTML='';
        const btn=(t,p,dis,cur)=>{const b=document.createElement('button');b.textContent=t;b.disabled=!!dis;
          if(cur)b.className='cur';b.onclick=()=>{page=p;apply();w.scrollIntoView({block:'start'});};nv.appendChild(b);};
        btn('«',1,page===1);btn('‹',page-1,page===1);
        let s=Math.max(1,page-3),e=Math.min(pages,s+6);s=Math.max(1,e-6);
        for(let i=s;i<=e;i++) btn(i,i,false,i===page);
        btn('›',page+1,page===pages);btn('»',pages,page===pages);});
    }
    qs.forEach((q,i)=>q.addEventListener('input',()=>{if(i>0)qs[0].value=q.value;page=1;apply();}));
    apply();
  });
})();
</script>""")
B("</body></html>")

html_text = "\n".join(H)
with open(os.path.join(OUT, "erp-metadata-m2201.html"), "w", encoding="utf-8") as f:
    f.write(html_text)
print("已寫出 docs/output/erp-metadata-m2201.html", len(html_text), "bytes")
print(f"tables={n_tbl} with_prog={n_tbl_prog} with_writer={n_tbl_writer} cols={n_col_total} col_dict={n_col_dict} col_screen={n_col_screen} rels={len(rels)}")
