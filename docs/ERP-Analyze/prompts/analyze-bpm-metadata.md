# Agent 執行 Prompt：解析 BPM（Agentflow / EFGP）Metadata 並產出關聯圖

> 本檔是交給 agent 直接執行的指令。格式參照 [analyze-erp-metadata.md](analyze-erp-metadata.md)，連線資訊見 [../DB-INFO.md](../DB-INFO.md)。

## 角色

你是一位資料庫逆向工程工程師，專長為 **Microsoft SQL Server** 資料庫結構分析，並熟悉華苓 **Agentflow BPM**（資料庫 `EFGP`）的表結構慣例。

## 目標

解析 BPM（Agentflow）測試站資料庫，整理出 **Table 與 Column 的 metadata（含中文名稱／說明）以及資料表之間的對應關係**，最後產出一份關聯圖文件（同時輸出 Markdown 與 HTML）。
...
1. 推測資料主要落在 `dbo` schema。請先列出所有 schema 與各 schema 的 table 數，確認是否有其他 schema（如 `sys` 以外的自訂 schema）也含業務資料。
2. 推測 Agentflow 表名有命名前綴規律（例如流程／組織／表單／簽核相關前綴），請以實際查得的表名前綴分群驗證，不可直接假設前綴含義。
3. 推測欄位中文名稱／說明來源優先序為：
   - `sys.extended_properties`（`name = 'MS_Description'`）的擴充屬性；
   - 若無，找 Agentflow 應用層的「資料字典／表單欄位定義」表（例如存放表單欄位、流程定義、組織人員對應的設定表）；
   - 皆無則退回欄位名本身。
...
2. 由命名慣例推導隱含關聯（BPM 常以流程序號、表單序號、人員代碼、組織代碼等共用欄位關聯，未必有實體 FK）。
...
兩份內容一致，需包含：
- **總覽**：資料庫角色、各 schema 與表分群（依前綴）的摘要與 table 數。
- **已知條件驗證**：逐項列出驗證結論。
